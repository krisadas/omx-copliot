# `omxc autoresearch` deep-interview UX review notes
...
- `omxc autoresearch` with no args enters `guidedAutoresearchSetup()` and immediately asks for a raw evaluator command.
...
- `src/cli/__tests__/autoresearch.test.ts`
  - still asserts the older `omxc autoresearch init [--topic ...]` help surface and non-interactive `mission-dir is required` failure.
...
### 8. Bare `omxc autoresearch init` documented as novice alias
**Fail on current branch.**
...
1. Update `README.md` with the new novice entry surfaces:
   - `omxc autoresearch` as a deep-interview-style refinement flow
