# `omxc autoresearch` parity contract

`omxc autoresearch` is a thin supervisor that drives one Codex experiment session per iteration while OMX owns the durable keep/discard/reset loop.

## CLI

```bash
omxc autoresearch <mission-dir> [codex-args...]
omxc autoresearch --resume <run-id> [codex-args...]
omxc autoresearch --help
```
