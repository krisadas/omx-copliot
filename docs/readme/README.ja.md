...
[GitHub Copilot CLI](https://github.com/openai/codex)のためのマルチエージェントオーケストレーションレイヤー。

## v0.9.0 の新機能 — Spark Initiative

Spark Initiative は、OMX のネイティブ探索・検査経路を強化するリリースです。

- **`omxc explore` ネイティブハーネス** — 読み取り専用のリポジトリ探索を Rust ベースのハーネスで高速かつ厳格に実行します。
- **`omxc sparkshell`** — 長い出力の要約と tmux pane キャプチャを行う、オペレーター向けのネイティブ検査サーフェスです。
...
```bash
omxc                # Codexを起動（tmuxでHUD付き）
omxc setup          # スコープ別にプロンプト/スキル/設定をインストール + プロジェクト .omx + スコープ別 AGENTS.md
omxc doctor         # インストール/ランタイム診断
omxc doctor --team  # Team/swarm診断
omxc team ...       # tmuxチームワーカーの開始/ステータス/再開/シャットダウン
omxc status         # アクティブなモードを表示
omxc cancel         # アクティブな実行モードをキャンセル
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test（プラグイン拡張ワークフロー）
...
OMXにはプラグインのスキャフォールディングとバリデーション用の`omxc hooks`が含まれるようになりました。

- `omxc tmux-hook`は引き続きサポートされ、変更されていません。
- `omxc hooks`は追加的であり、tmux-hookワークフローを置き換えません。
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # AGENTS.md注入を無効化
...
OMX_TEAM_WORKER_CLI=copilot-cli   # Copilot CLIワーカーを強制
OMX_TEAM_WORKER_CLI=claude  # Claude CLIワーカーを強制
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # ワーカーごとのCLIミックス（長さ=1またはワーカー数）
...
- 起動動作：永続化されたスコープが`project`の場合、`omxc`起動時に自動的に`CODEX_HOME=./.codex`を使用（`CODEX_HOME`が既に設定されている場合を除く）。
...
- **[CLIリファレンス](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — すべての`omxc`コマンド、フラグ、ツール
...
[oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)にインスパイアされ、Copilot CLI向けに適応されました。
