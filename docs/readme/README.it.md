...
Livello di orchestrazione multi-agente per [GitHub Copilot CLI](https://github.com/openai/codex).
...
- **Harness nativo per `omxc explore`** — esegue l’esplorazione del repository in sola lettura tramite un percorso Rust più rapido e più rigoroso.
- **`omxc sparkshell`** — superficie nativa per operatori con riepiloghi dell’output lungo e cattura esplicita dei pannelli tmux.
...
```bash
omxc                # Avvia Codex (+ HUD in tmux se disponibile)
omxc setup          # Installa prompt/skill/config per scope + .omxc del progetto + AGENTS.md specifico dello scope
omxc doctor         # Diagnostica installazione/esecuzione
omxc doctor --team  # Diagnostica Team/Swarm
omxc team ...       # Avvia/stato/riprendi/arresta i worker del team tmux
omxc status         # Mostra le modalità attive
omxc cancel         # Annulla le modalità di esecuzione attive
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test (workflow estensione plugin)
...
OMX ora include `omxc hooks` per lo scaffolding e la validazione dei plugin.

- `omxc tmux-hook` resta supportato e invariato.
- `omxc hooks` è additivo e non sostituisce i workflow tmux-hook.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # disabilita l'iniezione AGENTS.md
...
OMX_TEAM_WORKER_CLI=copilot-cli   # forza i worker Copilot CLI
OMX_TEAM_WORKER_CLI=claude  # forza i worker Claude CLI
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # mix CLI per worker (lunghezza=1 o numero di worker)
...
## Cosa scrive `omxc setup`

- `.omx/setup-scope.json` (scope di setup persistito)
- Installazioni dipendenti dallo scope:
  - `user`: `~/.codex/prompts/`, `~/.codex/skills/`, `~/.codex/config.toml`, `~/.omx/agents/`, `~/.codex/AGENTS.md`
  - `project`: `./.codex/prompts/`, `./.codex/skills/`, `./.codex/config.toml`, `./.omx/agents/`, `./AGENTS.md`
- Comportamento all'avvio: se lo scope persistito è `project`, l'avvio `omxc` usa automaticamente `CODEX_HOME=./.codex` (a meno che `CODEX_HOME` non sia già impostato).
...
- **[Riferimento CLI](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — Tutti i comandi `omxc`, flag e strumenti
...
Ispirato da [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode), adattato per Copilot CLI.
