...
[GitHub Copilot CLI](https://github.com/openai/codex) 的多智能体编排层。

## v0.9.0 新特性 — Spark Initiative

Spark Initiative 是一次强化 OMX 原生探索与检查路径的版本发布。

- **`omxc explore` 原生 harness** —— 通过 Rust 原生 harness 更快、更严格地执行只读仓库探索。
- **`omxc sparkshell`** —— 面向操作者的原生检查界面，支持长输出摘要与 tmux pane 捕获。
...
```bash
omxc                # 启动 Codex（在 tmux 中附带 HUD）
omxc setup          # 按作用域安装 prompt/skill/config + 项目 .omx + 作用域专属 AGENTS.md
omxc doctor         # 安装/运行时诊断
omxc doctor --team  # Team/swarm 诊断
omxc team ...       # 启动/状态/恢复/关闭 tmux 团队 worker
omxc status         # 显示活动模式
omxc cancel         # 取消活动执行模式
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test（插件扩展工作流）
...
OMX 现在包含用于插件脚手架和验证的 `omxc hooks`。

- `omxc tmux-hook` 继续支持且未更改。
- `omxc hooks` 是附加的，不会替代 tmux-hook 工作流。
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # 禁用 AGENTS.md 注入
...
OMX_TEAM_WORKER_CLI=copilot-cli   # 强制 Copilot CLI worker
OMX_TEAM_WORKER_CLI=claude  # 强制 Claude CLI worker
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # 每个 worker 的 CLI 混合（长度=1 或 worker 数量）
...
- 启动行为：如果持久化的作用域是 `project`，`omxc` 启动时自动使用 `CODEX_HOME=./.codex`（除非 `CODEX_HOME` 已设置）。
...
- **[CLI 参考](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — 所有 `omxc` 命令、标志和工具
...
受 [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode) 启发，为 Copilot CLI 适配。
