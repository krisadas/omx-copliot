...
Слой мультиагентной оркестрации для [GitHub Copilot CLI](https://github.com/openai/codex).

## Что нового в v0.9.0 — Spark Initiative

Spark Initiative — это релиз, усиливающий нативный путь исследования и инспекции в OMX.

- **Нативный harness для `omxc explore`** — ускоряет и ужесточает read-only исследование репозитория через Rust-путь.
- **`omxc sparkshell`** — нативная операторская поверхность для инспекции с краткими сводками длинного вывода и явным захватом tmux-pane.
...
```bash
omxc                # Запустить Codex (+ HUD в tmux при наличии)
omxc setup          # Установить промпты/навыки/конфиг по области + .omx проекта + AGENTS.md для выбранной области
omxc doctor         # Диагностика установки/среды выполнения
omxc doctor --team  # Диагностика Team/swarm
omxc team ...       # Запуск/статус/возобновление/завершение рабочих tmux
omxc status         # Показать активные режимы
omxc cancel         # Отменить активные режимы выполнения
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test (рабочий процесс расширений плагинов)
...
OMX теперь включает `omxc hooks` для создания шаблонов плагинов и валидации.

- `omxc tmux-hook` по-прежнему поддерживается и не изменён.
- `omxc hooks` является дополнительным и не заменяет рабочие процессы tmux-hook.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # отключить внедрение AGENTS.md
...
OMX_TEAM_WORKER_CLI=copilot-cli   # принудительно Copilot CLI
OMX_TEAM_WORKER_CLI=claude  # принудительно Claude CLI
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # CLI для каждого рабочего (длина=1 или количество рабочих)
...
## Что записывает `omxc setup`

- `.omx/setup-scope.json` (сохранённая область установки)
- Установки в зависимости от области:
  - `user`: `~/.codex/prompts/`, `~/.codex/skills/`, `~/.codex/config.toml`, `~/.omx/agents/`, `~/.codex/AGENTS.md`
  - `project`: `./.codex/prompts/`, `./.codex/skills/`, `./.codex/config.toml`, `./.omx/agents/`, `./AGENTS.md`
- Поведение при запуске: если сохранённая область — `project`, `omxc` автоматически использует `CODEX_HOME=./.codex` (если `CODEX_HOME` ещё не задан).
...
- **[Справочник CLI](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — Все команды `omxc`, флаги и инструменты
...
Вдохновлено проектом [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode), адаптировано для Copilot CLI.
