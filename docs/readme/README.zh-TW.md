...
[GitHub Copilot CLI](https://github.com/openai/codex) 的多智能體編排層。

## v0.9.0 新功能 — Spark Initiative

Spark Initiative 是一個強化 OMX 原生探索與檢查路徑的版本發布。

- **`omxc explore` 原生 harness** —— 以 Rust 原生 harness 更快且更嚴格地執行唯讀儲存庫探索。
- **`omxc sparkshell`** —— 面向操作員的原生檢查介面，支援長輸出摘要與 tmux pane 擷取。
...
```bash
omxc                  # 啟動 Codex（可用時在 tmux 中附帶 HUD）
omxc setup            # 依範圍安裝提示詞/技能/設定 + 專案 .omx + 範圍專屬 AGENTS.md
omxc doctor           # 安裝/執行期診斷
omxc doctor --team    # 團隊/群集診斷
omxc ask ...          # 詢問本地供應商顧問（claude|gemini），結果寫入 .omx/artifacts/*
omxc team ...         # 啟動/狀態/恢復/關閉團隊工作進程（預設為互動式 tmux）
omxc status           # 顯示目前活動模式
omxc cancel           # 取消活動中的執行模式
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...    # init|status|validate|test
omxc hooks ...        # init|status|validate|test（插件擴充工作流程）
...
```bash
omxc ask claude "review this diff"
omxc ask gemini "brainstorm alternatives"
omxc ask claude --agent-prompt executor "implement feature X with tests"
omxc ask gemini --agent-prompt=planner --prompt "draft a rollout plan"
# 底層供應商 CLI 說明中的旗標：
...
```bash
OMX_TEAM_WORKER_LAUNCH_MODE=prompt omxc team 2:executor "task"
```

## Hooks 擴充（附加介面）

OMX 現已包含 `omxc hooks`，用於插件鷹架建立與驗證。

- `omxc tmux-hook` 持續受支援，行為不變。
- `omxc hooks` 屬於附加功能，不會取代 tmux-hook 工作流程。
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # 停用 AGENTS.md 注入
...
請在 Team 工作完成後另外執行 `omxc ralph ...`。舊的 linked-Ralph 團隊路徑已不再是建議或支援的標準路徑。

團隊工作進程的 Worker CLI 選擇：

```bash
OMX_TEAM_WORKER_CLI=auto    # 預設；當 worker --model 包含 "claude" 時使用 claude
OMX_TEAM_WORKER_CLI=copilot-cli   # 強制使用 Copilot CLI 工作進程
OMX_TEAM_WORKER_CLI=claude  # 強制使用 Claude CLI 工作進程
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # 每個工作進程的 CLI 混合（長度為 1 或等於工作進程數量）
...
- 啟動行為：若持久化範圍為 `project`，`omxc` 啟動時自動使用 `CODEX_HOME=./.codex`（除非已設定 `CODEX_HOME`）。
...
- **[CLI 參考手冊](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — 所有 `omxc` 指令、旗標與工具
...
靈感來自 [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)，為 Copilot CLI 量身改編。
