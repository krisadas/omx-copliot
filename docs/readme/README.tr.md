...
[GitHub Copilot CLI](https://github.com/openai/codex) için çok ajanlı orkestrasyon katmanı.
...
- **`omxc explore` için native harness** — salt okunur depo keşfini Rust tabanlı daha hızlı ve daha sıkı bir yol üzerinden çalıştırır.
- **`omxc sparkshell`** — uzun çıktıları özetleyen ve açık tmux pane yakalama desteği veren operatör odaklı native inceleme yüzeyidir.
...
```bash
omxc                # Codex'i başlat (tmux'ta HUD ile birlikte)
omxc setup          # Prompt/skill/config'i kapsama göre kur + proje .omx + kapsama özel AGENTS.md
omxc doctor         # Kurulum/çalışma zamanı tanılamaları
omxc doctor --team  # Team/swarm tanılamaları
omxc team ...       # tmux takım çalışanlarını başlat/durum/devam et/kapat
omxc status         # Aktif modları göster
omxc cancel         # Aktif çalışma modlarını iptal et
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test (eklenti uzantı iş akışı)
...
OMX artık eklenti iskelesi ve doğrulaması için `omxc hooks` içerir.

- `omxc tmux-hook` desteklenmeye devam eder ve değişmemiştir.
- `omxc hooks` ek niteliktedir ve tmux-hook iş akışlarını değiştirmez.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # AGENTS.md enjeksiyonunu devre dışı bırak
...
OMX_TEAM_WORKER_CLI=copilot-cli   # Copilot CLI çalışanlarını zorla
OMX_TEAM_WORKER_CLI=claude  # Claude CLI çalışanlarını zorla
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # çalışan başına CLI karışımı (uzunluk=1 veya çalışan sayısı)
...
- Başlatma davranışı: kalıcı kapsam `project` ise, `omxc` başlatma otomatik olarak `CODEX_HOME=./.codex` kullanır (`CODEX_HOME` zaten ayarlanmadıysa).
...
- **[CLI Referansı](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — Tüm `omxc` komutları, bayraklar ve araçlar
...
[oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)'dan ilham alınmıştır, Copilot CLI için uyarlanmıştır.
