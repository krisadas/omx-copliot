...
Capa de orquestación multiagente para [GitHub Copilot CLI](https://github.com/openai/codex).
...
- **Harness nativo para `omxc explore`** — ejecuta exploración de repositorio en modo solo lectura con una vía Rust más rápida y más estricta.
- **`omxc sparkshell`** — superficie nativa para operadores, con resúmenes de salidas largas y captura explícita de paneles tmux.
...
```bash
omxc                # Lanzar Codex (+ HUD en tmux cuando está disponible)
omxc setup          # Instalar prompts/skills/config por alcance + .omxc del proyecto + AGENTS.md específico del alcance
omxc doctor         # Diagnósticos de instalación/ejecución
omxc doctor --team  # Diagnósticos de Team/swarm
omxc team ...       # Iniciar/estado/reanudar/apagar workers tmux del equipo
omxc status         # Mostrar modos activos
omxc cancel         # Cancelar modos de ejecución activos
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test (flujo de trabajo de extensión de plugins)
...
OMX ahora incluye `omxc hooks` para scaffolding y validación de plugins.

- `omxc tmux-hook` sigue siendo compatible y no ha cambiado.
- `omxc hooks` es aditivo y no reemplaza los flujos de trabajo de tmux-hook.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # desactivar inyección de AGENTS.md
...
OMX_TEAM_WORKER_CLI=copilot-cli   # forzar workers Copilot CLI
OMX_TEAM_WORKER_CLI=claude  # forzar workers Claude CLI
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # mezcla de CLI por worker (longitud=1 o cantidad de workers)
...
- Comportamiento de inicio: si el alcance persistido es `project`, el lanzamiento de `omxc` usa automáticamente `CODEX_HOME=./.codex` (a menos que `CODEX_HOME` ya esté establecido).
...
- **[Referencia CLI](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — Todos los comandos `omxc`, flags y herramientas
...
Inspirado en [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode), adaptado para Copilot CLI.
