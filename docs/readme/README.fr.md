...
Couche d'orchestration multi-agents pour [GitHub Copilot CLI](https://github.com/openai/codex).
...
- **Harness natif pour `omxc explore`** — exécute l’exploration read-only du dépôt via une voie Rust plus rapide et plus stricte.
- **`omxc sparkshell`** — surface native orientée opérateur, avec résumés de sorties longues et capture explicite de panneaux tmux.
...
```bash
omxc                # Lancer Codex (+ HUD dans tmux si disponible)
omxc setup          # Installer prompts/skills/config par scope + .omxc du projet + AGENTS.md propre au scope
omxc doctor         # Diagnostics d'installation/exécution
omxc doctor --team  # Diagnostics Team/Swarm
omxc team ...       # Démarrer/statut/reprendre/arrêter les workers d'équipe tmux
omxc status         # Afficher les modes actifs
omxc cancel         # Annuler les modes d'exécution actifs
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test (workflow d'extension de plugins)
...
OMX inclut désormais `omxc hooks` pour l'échafaudage et la validation de plugins.

- `omxc tmux-hook` reste supporté et inchangé.
- `omxc hooks` est additif et ne remplace pas les workflows tmux-hook.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # désactiver l'injection AGENTS.md
...
OMX_TEAM_WORKER_CLI=copilot-cli   # forcer les workers Copilot CLI
OMX_TEAM_WORKER_CLI=claude  # forcer les workers Claude CLI
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # mix CLI par worker (longueur=1 ou nombre de workers)
...
## Ce que `omxc setup` écrit

- `.omx/setup-scope.json` (scope de setup persisté)
- Installations dépendantes du scope :
  - `user` : `~/.codex/prompts/`, `~/.codex/skills/`, `~/.codex/config.toml`, `~/.omx/agents/`, `~/.codex/AGENTS.md`
  - `project` : `./.codex/prompts/`, `./.codex/skills/`, `./.codex/config.toml`, `./.omx/agents/`, `./AGENTS.md`
- Comportement au lancement : si le scope persisté est `project`, le lancement `omxc` utilise automatiquement `CODEX_HOME=./.codex` (sauf si `CODEX_HOME` est déjà défini).
...
- **[Référence CLI](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — Toutes les commandes `omxc`, flags et outils
...
Inspiré par [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode), adapté pour Copilot CLI.
