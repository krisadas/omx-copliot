...
OMX to nakładka na [GitHub Copilot CLI](https://github.com/openai/codex).
...
- Node.js 20+
- Copilot CLI: `npm install -g @openai/codex`
...
1. Uruchom `omxc setup`
2. Wystartuj z `omxc --madmax --high`
...
- `omxc setup` — instaluje prompty, skille, konfigurację i strukturę plików AGENTS
- `omxc doctor` — sprawdza instalację, gdy coś nie działa
- `omxc hud --watch` — podgląd stanu i postępu, nie główny cykl pracy

### Explore i sparkshell

- `omxc explore --prompt "..."` — przeszukuje repo tylko do odczytu
- `omxc sparkshell <command>` — inspekcja w shellu i ograniczona weryfikacja
...
`omxc team` wymaga tmux lub odpowiednika:
