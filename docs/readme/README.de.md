...
Multi-Agenten-Orchestrierungsschicht für [GitHub Copilot CLI](https://github.com/openai/codex).
...
- **Nativer Harness für `omxc explore`** — führt Read-only-Repository-Exploration über einen schnelleren und strengeren Rust-Pfad aus.
- **`omxc sparkshell`** — native Operator-Oberfläche für Inspektion mit Zusammenfassungen langer Ausgaben und expliziter tmux-Pane-Erfassung.
...
```bash
omxc                # Codex starten (+ HUD in tmux wenn verfügbar)
omxc setup          # Prompts/Skills/Config nach Bereich installieren + Projekt-.omx + bereichsspezifische AGENTS.md
omxc doctor         # Installations-/Laufzeitdiagnose
omxc doctor --team  # Team/Swarm-Diagnose
omxc team ...       # tmux-Team-Worker starten/Status/fortsetzen/herunterfahren
omxc status         # Aktive Modi anzeigen
omxc cancel         # Aktive Ausführungsmodi abbrechen
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test (Plugin-Erweiterungs-Workflow)
...
OMX enthält jetzt `omxc hooks` für Plugin-Gerüstbau und -Validierung.

- `omxc tmux-hook` wird weiterhin unterstützt und ist unverändert.
- `omxc hooks` ist additiv und ersetzt keine tmux-hook-Workflows.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # AGENTS.md-Injektion deaktivieren
...
OMX_TEAM_WORKER_CLI=copilot-cli   # Codex-CLI-Worker erzwingen
OMX_TEAM_WORKER_CLI=claude  # Claude-CLI-Worker erzwingen
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # CLI-Mix pro Worker (Länge=1 oder Worker-Anzahl)
...
## Was `omxc setup` schreibt

- `.omx/setup-scope.json` (persistierter Setup-Bereich)
- Bereichsabhängige Installationen:
  - `user`: `~/.codex/prompts/`, `~/.codex/skills/`, `~/.codex/config.toml`, `~/.omx/agents/`, `~/.codex/AGENTS.md`
  - `project`: `./.codex/prompts/`, `./.codex/skills/`, `./.codex/config.toml`, `./.omx/agents/`, `./AGENTS.md`
- Startverhalten: Wenn der persistierte Bereich `project` ist, verwendet `omxc` automatisch `CODEX_HOME=./.codex` (sofern `CODEX_HOME` nicht bereits gesetzt ist).
...
- **[CLI-Referenz](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — Alle `omxc`-Befehle, Flags und Tools
...
Inspiriert von [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode), angepasst für Copilot CLI.
