...
OMX là lớp workflow mở rộng cho [GitHub Copilot CLI](https://github.com/openai/codex).
...
- Node.js 20+
- Copilot CLI đã cài: `npm install -g @openai/codex`
...
1. Chạy `omxc setup`
2. Khởi động với `omxc --madmax --high`
...
- `omxc setup` cài prompt, skill, config và scaffold AGENTS
- `omxc doctor` kiểm tra cài đặt khi có vấn đề
- `omxc hud --watch` theo dõi trạng thái, không phải workflow chính

### Explore và sparkshell

- `omxc explore --prompt "..."` tìm kiếm chỉ đọc trong repository
- `omxc sparkshell <command>` chạy lệnh shell có kiểm soát output
...
`omxc team` cần backend tương thích tmux:
