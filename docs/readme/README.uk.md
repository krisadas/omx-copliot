...
OMX — це шар робочих процесів для [GitHub Copilot CLI](https://github.com/openai/codex).
...
- Node.js 20+
- встановлений Copilot CLI: `npm install -g @openai/codex`
...
1. Запустіть `omxc setup`
2. Запустіть з `omxc --madmax --high`
...
```bash
omxc team 3:executor "fix the failing tests with verification"
...
- `omxc setup` встановлює промпти, навички, конфіг і структуру AGENTS
- `omxc doctor` перевіряє встановлення, коли щось здається не так
- `omxc hud --watch` — це інструмент моніторингу стану, а не основний робочий процес користувача

### Explore і sparkshell

- `omxc explore --prompt "..."` призначений для пошуку в репозиторії лише для читання
- `omxc sparkshell <command>` призначений для перевірки в терміналі та обмеженої валідації

Приклади:

```bash
omxc explore --prompt "find where team state is written"
omxc sparkshell git status
omxc sparkshell --tmux-pane %12 --tail-lines 400
```
...
- `omxc setup` має увімкнути нативні хуки Codex (`[features].codex_hooks = true`) у
...
- `omxc tmux-hook` зарезервовано для поведінки командного рушія та усунення проблем із застарілим tmux
...
`omxc team` потребує інструмента, сумісного з tmux:
...
- `xattr -dr com.apple.quarantine $(which omxc)`
