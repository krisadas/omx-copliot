...
[GitHub Copilot CLI](https://github.com/openai/codex)를 위한 멀티 에이전트 오케스트레이션 레이어.

## v0.9.0 새로운 기능 — Spark Initiative

Spark Initiative는 OMX의 네이티브 탐색/검사 경로를 강화한 릴리스입니다.

- **`omxc explore` 네이티브 하네스** — 읽기 전용 저장소 탐색을 Rust 기반 하네스로 더 빠르고 엄격하게 실행합니다.
- **`omxc sparkshell`** — 긴 출력을 요약하고 tmux pane 캡처를 지원하는 운영자용 네이티브 검사 표면입니다.
...
```bash
omxc                # Codex 실행 (tmux에서 HUD와 함께)
omxc setup          # 범위별 프롬프트/스킬/설정 설치 + 프로젝트 .omx + 범위별 AGENTS.md
omxc doctor         # 설치/런타임 진단
omxc doctor --team  # Team/swarm 진단
omxc team ...       # tmux 팀 워커 시작/상태/재개/종료
omxc status         # 활성 모드 표시
omxc cancel         # 활성 실행 모드 취소
omxc reasoning <mode> # low|medium|high|xhigh
omxc tmux-hook ...  # init|status|validate|test
omxc hooks ...      # init|status|validate|test (플러그인 확장 워크플로우)
...
OMX는 이제 플러그인 스캐폴딩 및 검증을 위한 `omxc hooks`를 포함합니다.

- `omxc tmux-hook`은 계속 지원되며 변경되지 않았습니다.
- `omxc hooks`는 추가적이며 tmux-hook 워크플로우를 대체하지 않습니다.
...
```bash
OMX_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omxc     # AGENTS.md 주입 비활성화
...
OMX_TEAM_WORKER_CLI=copilot-cli   # Copilot CLI 워커 강제
OMX_TEAM_WORKER_CLI=claude  # Claude CLI 워커 강제
OMX_TEAM_WORKER_CLI_MAP=copilot-cli,copilot-cli,claude,claude  # 워커별 CLI 혼합 (길이=1 또는 워커 수)
...
- 시작 동작: 저장된 범위가 `project`이면, `omxc` 시작 시 자동으로 `CODEX_HOME=./.codex`를 사용합니다 (`CODEX_HOME`이 이미 설정되지 않은 경우).
...
- **[CLI 레퍼런스](https://yeachan-heo.github.io/oh-my-codex-website/docs.html#cli-reference)** — 모든 `omxc` 명령어, 플래그 및 도구
...
[oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)에서 영감을 받아 Copilot CLI용으로 적응하였습니다.
