...
- use `omxc setup` and need predictable install scope behavior,
- run `omxc team`/tmux workflows and want the latest reliability fixes,
...
- `omxc setup` now supports scope-aware install modes (`user`, `project`). Legacy `project-local` values are auto-migrated.
...
| `$deepinit` | `omxc agents-init [path]` | Lightweight CLI successor for AGENTS.md bootstrap only; immediate child directories only, unmanaged files preserved unless `--force`. |
...
- [ ] If using spark worker routing, verify flags are available:
  ```bash
  omxc --help | rg "spark|madmax-spark"
  ```
