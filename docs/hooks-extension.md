...
- `.codex/hooks.json` = native Codex hook registrations installed by `omxc setup`
- `.omx/hooks/*.mjs` = OMX plugin hooks dispatched by runtime/native events
- `omxc tmux-hook` / notify-hook / derived watcher = tmux/runtime fallback surfaces

> Compatibility guarantee: `omxc tmux-hook` remains fully supported and unchanged.
> The new `omxc hooks` command group is additive and does **not** replace tmux-hook workflows.
...
- `omxc tmux-hook` remains a CLI/runtime workflow, not `sdk.omx.tmuxHook.*`
