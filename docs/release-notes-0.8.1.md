...
If you use `$team`/`omxc team` heavily, this release makes the runtime more consistent by standardizing around **CLI-first interop**.
...
- If you previously relied on legacy `team_*` MCP workflows, migrate to:
  - `omxc team api <operation> ...`
- For notification setup, prefer:
  - `omxc configure-notifications` (or skill equivalent)
