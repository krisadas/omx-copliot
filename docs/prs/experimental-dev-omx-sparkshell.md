...
## Summary
This PR rebases the `omxc sparkshell` feature line onto `experimental/dev` and preserves the follow-on team-status inspection work built on top of it.

It adds the native Rust-backed `omxc sparkshell <command> [args...]` flow, plus a long sequence of `omxc team status` improvements that surface pane-aware inspection metadata and ready-to-run sparkshell commands for leader triage.

## What’s included
### `omxc sparkshell`
- adds CLI dispatch/help for `omxc sparkshell`
...
### `omxc team status` inspection improvements
