# PR Draft: Deprecate `omxc team ralph` and keep team standalone

## Target branch
`dev`

## Title
Deprecate `omxc team ralph` and remove linked team↔Ralph lifecycle machinery

## Summary
This PR removes the built-in linked `omxc team ralph` workflow and restores a clean separation between `team` and `ralph`.

After this change:
- `omxc team ...` / `$team ...` is the only supported team launch path
- `omxc ralph ...` / `$ralph ...` remains available as a separate, explicit follow-up
- team no longer creates, syncs, or depends on linked Ralph state
- legacy `omxc team ralph ...` usage now fails with a clear deprecation error instead of being tolerated silently
...
- advertised/generated `omxc team ralph ...` launch hints
- linked-Ralph-specific tests and compatibility paths inside team runtime

### Updated
- team help/usage text now documents only `omxc team [N:agent-type] "<task>"`
...
- follow-up planner and pipeline launch hints now generate plain `omxc team ...`
- shutdown/resume/state flows now operate on standalone team semantics only

### Behavior change
- `omxc team ralph ...` is now rejected with an explicit deprecation error:
  - use `omxc team ...` for coordinated team execution
  - use `omxc ralph ...` separately if a later persistent follow-up loop is still needed
...
## Impact
### Before
- `omxc team ralph ...` had special runtime behavior
...
### After
- `omxc team ...` runs coordinated team execution only
- team owns its own verification lanes and shutdown evidence
- `omxc ralph ...` is separate and explicit
...
## Risks / notes
- This is an intentional CLI behavior break for users still invoking `omxc team ralph ...`
