...
### Spark Initiative: `omxc explore` and `omxc sparkshell`

OMX now has a stronger native fast path for repository discovery and shell-native inspection.

This release:
- introduces `omxc explore` as the default read-only exploration entrypoint
- adds the Rust-backed explore harness plus packaging and source-fallback flow
- introduces `omxc sparkshell <command> [args...]` as an explicit operator-facing native sidecar
- allows qualifying read-only shell-native `omxc explore` tasks to route through `omxc sparkshell`
- keeps the explore path intentionally constrained: shell-only, read-only, and allowlisted

Representative changes:
- `fb07c3c` — feat: add omxc explore harness and packaging flow
- `71858c3` — feat: add omxc sparkshell and team inspection metadata
...
This release:
- exposes `omxc sparkshell --tmux-pane <pane-id> --tail-lines <100-1000>` for explicit pane summarization
...
```bash
omxc setup --force --scope project
```

- Expect `omxc explore` and `omxc sparkshell` packaged installs to rely on release-asset hydration when no explicit binary override or repo-local artifact is present.
