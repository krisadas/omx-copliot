...
After `omxc setup`, native agent configs in `~/.omx/agents/` include new sections:
...
2f4862a docs(omxc): remove remaining legacy model alias references
0d2115c fix(omxc): remove legacy model aliases from prompts and runtime metadata
8fb3aa0 fix(bin): use file:// URL for dynamic import on Windows
f448108 feat(omxc): add posture-aware agent routing metadata
```
