...
- Node.js >= 20
- [GitHub Copilot CLI](https://github.com/github/copilot-cli) installed (`npm install -g @githubnext/github-copilot-cli` or via Homebrew)
- GitHub Copilot access configured
...
Setup complete! Run "omxc doctor" to verify installation.
```
...
**Expected output for `omxc version`:**
```
omxc vX.Y.Z
Node.js v20+
Platform: linux x64
...
**Expected output for `omxc status` (no active modes):**
```
No active modes.
```
...
export OMX_TEAM_WORKER_CLI_MAP=copilot,copilot,copilot,claude,claude,claude
export OMX_TEAM_WORKER_LAUNCH_ARGS='--effort low'

# 5-worker baseline
omxc team 5:executor "parallel team smoke"

# 6-worker mixed-CLI E2E run
omxc team 6:executor "$TEAM_TASK"
...
# Lifecycle commands
omxc team status "$TEAM_NAME"
omxc team resume "$TEAM_NAME"
omxc team shutdown "$TEAM_NAME"
```
...
## Demo 7: `omxc team api` Rich CLI Interop Demonstration

All mutations should use CLI interop (`omxc team api ... --json`) with the stable JSON envelope.

### 7.1 Task lifecycle (claim-safe)

```bash
CREATE_JSON=$(omxc team api create-task --input '{"team_name":"e2e-team-demo","subject":"Demo lifecycle","description":"Claim-safe lifecycle demo","owner":"worker-1"}' --json)
TASK_ID=$(echo "$CREATE_JSON" | jq -r '.data.task.id')

CLAIM_JSON=$(omxc team api claim-task --input "{\"team_name\":\"e2e-team-demo\",\"task_id\":\"$TASK_ID\",\"worker\":\"worker-1\",\"expected_version\":1}" --json)
CLAIM_TOKEN=$(echo "$CLAIM_JSON" | jq -r '.data.claimToken')

omxc team api transition-task-status --input "{\"team_name\":\"e2e-team-demo\",\"task_id\":\"$TASK_ID\",\"from\":\"in_progress\",\"to\":\"completed\",\"claim_token\":\"$CLAIM_TOKEN\"}" --json
```

### 7.2 Mailbox/message flow

```bash
omxc team api send-message --input '{"team_name":"e2e-team-demo","from_worker":"leader-fixed","to_worker":"worker-1","body":"ACK: worker-1 ready"}' --json
omxc team api broadcast --input '{"team_name":"e2e-team-demo","from_worker":"leader-fixed","body":"Sync checkpoint"}' --json
MAILBOX_JSON=$(omxc team api mailbox-list --input '{"team_name":"e2e-team-demo","worker":"worker-1"}' --json)
MESSAGE_ID=$(echo "$MAILBOX_JSON" | jq -r '.data.messages[0].message_id // empty')
omxc team api mailbox-mark-notified --input "{\"team_name\":\"e2e-team-demo\",\"worker\":\"worker-1\",\"message_id\":\"$MESSAGE_ID\"}" --json
omxc team api mailbox-mark-delivered --input "{\"team_name\":\"e2e-team-demo\",\"worker\":\"worker-1\",\"message_id\":\"$MESSAGE_ID\"}" --json
```

### 7.3 Complete operations matrix (broad coverage)

```bash
omxc team api read-task --input '{"team_name":"e2e-team-demo","task_id":"<TASK_ID>"}' --json
omxc team api list-tasks --input '{"team_name":"e2e-team-demo"}' --json
omxc team api update-task --input '{"team_name":"e2e-team-demo","task_id":"<TASK_ID>","description":"Updated via CLI interop"}' --json
omxc team api release-task-claim --input '{"team_name":"e2e-team-demo","task_id":"<TASK_ID>","claim_token":"<CLAIM_TOKEN>","worker":"worker-1"}' --json
omxc team api read-config --input '{"team_name":"e2e-team-demo"}' --json
omxc team api read-manifest --input '{"team_name":"e2e-team-demo"}' --json
omxc team api read-worker-status --input '{"team_name":"e2e-team-demo","worker":"worker-1"}' --json
omxc team api read-worker-heartbeat --input '{"team_name":"e2e-team-demo","worker":"worker-1"}' --json
omxc team api update-worker-heartbeat --input '{"team_name":"e2e-team-demo","worker":"worker-1","pid":12345,"turn_count":12,"alive":true}' --json
omxc team api write-worker-inbox --input '{"team_name":"e2e-team-demo","worker":"worker-1","content":"# Inbox update\nProceed with task 2."}' --json
omxc team api write-worker-identity --input '{"team_name":"e2e-team-demo","worker":"worker-9","index":9,"role":"executor"}' --json
omxc team api append-event --input '{"team_name":"e2e-team-demo","type":"task_completed","worker":"worker-1","task_id":"<TASK_ID>","reason":"demo"}' --json
omxc team api get-summary --input '{"team_name":"e2e-team-demo"}' --json
omxc team api write-shutdown-request --input '{"team_name":"e2e-team-demo","worker":"worker-1","requested_by":"leader-fixed"}' --json
omxc team api read-shutdown-ack --input '{"team_name":"e2e-team-demo","worker":"worker-1"}' --json
omxc team api read-monitor-snapshot --input '{"team_name":"e2e-team-demo"}' --json
omxc team api write-monitor-snapshot --input '{"team_name":"e2e-team-demo","snapshot":{"taskStatusById":{"1":"completed"},"workerAliveByName":{"worker-1":true},"workerStateByName":{"worker-1":"idle"},"workerTurnCountByName":{"worker-1":12},"workerTaskIdByName":{"worker-1":"1"},"mailboxNotifiedByMessageId":{},"completedEventTaskIds":{"1":true}}}' --json
omxc team api read-task-approval --input '{"team_name":"e2e-team-demo","task_id":"<TASK_ID>"}' --json
omxc team api write-task-approval --input '{"team_name":"e2e-team-demo","task_id":"<TASK_ID>","status":"approved","reviewer":"leader-fixed","decision_reason":"demo approval","required":true}' --json
omxc team api cleanup --input '{"team_name":"e2e-team-demo"}' --json
```
...
omxc team api get-summary --input '{"team_name":"e2e-team-demo"}' --json | jq -e '.schema_version == "1.0" and .operation == "get-summary" and (.ok == true or .ok == false)'

# Team lifecycle checks
omxc team status "e2e-team-demo"
omxc team shutdown "e2e-team-demo"
```

Success criteria:
- All `omxc team api` examples return valid JSON envelopes.
...
- Team lifecycle demonstrates `omxc team`, `omxc team status`, `omxc team resume`, and `omxc team shutdown`.
...
CREATE_JSON=$(omxc team api create-task --input "{\"team_name\":\"$TEAM_NAME\",\"subject\":\"one-shot lifecycle\",\"description\":\"demo task\",\"owner\":\"worker-1\"}" --json)
TASK_ID=$(echo "$CREATE_JSON" | jq -r '.data.task.id')

echo "[4/8] claim task"
CLAIM_JSON=$(omxc team api claim-task --input "{\"team_name\":\"$TEAM_NAME\",\"task_id\":\"$TASK_ID\",\"worker\":\"worker-1\",\"expected_version\":1}" --json)
CLAIM_TOKEN=$(echo "$CLAIM_JSON" | jq -r '.data.claimToken')

echo "[5/8] transition task -> completed"
omxc team api transition-task-status --input "{\"team_name\":\"$TEAM_NAME\",\"task_id\":\"$TASK_ID\",\"from\":\"in_progress\",\"to\":\"completed\",\"claim_token\":\"$CLAIM_TOKEN\"}" --json

echo "[6/8] mailbox flow"
omxc team api send-message --input "{\"team_name\":\"$TEAM_NAME\",\"from_worker\":\"leader-fixed\",\"to_worker\":\"worker-1\",\"body\":\"ACK one-shot\"}" --json
MAILBOX_JSON=$(omxc team api mailbox-list --input "{\"team_name\":\"$TEAM_NAME\",\"worker\":\"worker-1\"}" --json)
MESSAGE_ID=$(echo "$MAILBOX_JSON" | jq -r '.data.messages[0].message_id // empty')
omxc team api mailbox-mark-notified --input "{\"team_name\":\"$TEAM_NAME\",\"worker\":\"worker-1\",\"message_id\":\"$MESSAGE_ID\"}" --json
omxc team api mailbox-mark-delivered --input "{\"team_name\":\"$TEAM_NAME\",\"worker\":\"worker-1\",\"message_id\":\"$MESSAGE_ID\"}" --json

echo "[7/8] summary envelope check"
omxc team api get-summary --input "{\"team_name\":\"$TEAM_NAME\"}" --json | jq -e '.schema_version == "1.0" and .operation == "get-summary" and .ok == true'

echo "[8/8] shutdown + cleanup"
omxc team shutdown "$TEAM_NAME"
omxc team api cleanup --input "{\"team_name\":\"$TEAM_NAME\"}" --json
...
| CLI commands | 11+ | `omxc (launch), setup, doctor, team, version, tmux-hook, hud, status, cancel, reasoning, help` |
| AGENTS.md | 1 | Project root (generated) |

## Troubleshooting

**Copilot CLI not found:** Install from [GitHub Copilot CLI](https://github.com/github/copilot-cli) or via `npm install -g @githubnext/github-copilot-cli`

**Slash commands not appearing:** Run `omxc setup --force` to reinstall prompts

**MCP servers not connecting:** Check `~/.copilot/config.toml` for `[mcp_servers.omx_state]`, `[mcp_servers.omx_memory]`, `[mcp_servers.omx_code_intel]`, and `[mcp_servers.omx_trace]` entries

**Doctor shows warnings:** Run `omxc setup` to install missing components

---

## Demo 9: Autoresearch Showcase Hub

OMX includes a lightweight research-showcase hub for reproducible autoresearch demos under `playground/README.md`.
...
| `OMX_TEAM_WORKER_LAUNCH_ARGS` | `--effort low` | Arguments passed to worker CLIs (worker model falls back to `OMX_DEFAULT_SPARK_MODEL`) |
...
OMX_TEAM_WORKER_CLI_MAP=copilot,copilot,copilot,copilot,claude,claude,claude,claude \
./scripts/demo-team-e2e.sh
```
