...
Meta-skill for managing omx-copilot skills via CLI-like commands.
...
2. Scan project skills at `.copilot/skills/`
...
✓ Created skill at ~/.copilot/skills/custom-logger/SKILL.md
...
```
Skill: custom-logger
Scope: user (~/.copilot/skills/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.copilot/skills/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.copilot/skills/custom-logger/SKILL.md
...
User: yes
Assistant: ✓ Copied 'error-handler' to .copilot/skills/
...
echo "=== USER-LEVEL SKILLS (~/.copilot/skills/) ==="
if [ -d "$HOME/.copilot/skills" ]; then
  USER_COUNT=$(find "$HOME/.copilot/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "$HOME/.copilot/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.copilot/skills/) ==="
if [ -d ".copilot/skills" ]; then
  PROJECT_COUNT=$(find ".copilot/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".copilot/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
Then ask for scope:
- **User-level** (~/.copilot/skills/) - Available across all projects
- **Project-level** (.copilot/skills/) - Only for this project
...
**Version Control**: Project-level skills (.copilot/skills/) are committed with your code, so the whole team benefits.
...
✓ User skills directory exists: ~/.copilot/skills/
✓ Project skills directory exists: .copilot/skills/
