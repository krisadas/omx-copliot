...
if (!existsSync(manifestPath)) {
  console.error(`omxc sparkshell build: missing Rust manifest at ${manifestPath}`);
  process.exit(1);
}
...
if (result.error) {
  console.error(`omxc sparkshell build: failed to launch cargo: ${result.error.message}`);
  process.exit(1);
}
...
if (!existsSync(releaseBinaryPath)) {
  console.error(`omxc sparkshell build: expected release binary at ${releaseBinaryPath}`);
  process.exit(1);
}
...
console.log(`omxc sparkshell build: staged native binary at ${packagedBinaryPath}`);
