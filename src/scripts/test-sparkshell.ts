...
if (!existsSync(manifestPath)) {
  console.error(`omxc sparkshell test: missing Rust manifest at ${manifestPath}`);
  process.exit(1);
}
...
if (result.error) {
  console.error(`omxc sparkshell test: failed to launch cargo: ${result.error.message}`);
  process.exit(1);
}
