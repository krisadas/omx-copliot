...
describe("codex native hook dispatch", () => {
    ...
    try {
      ...
      assert.deepEqual(result.outputJson, {
        hookSpecificOutput: {
          ...
          additionalContext:
            "OMX native SessionStart detected. Load workspace conventions from AGENTS.md, restore relevant .omxc runtime/project memory context, and continue from existing mode state before making changes.",
        },
      });
  ...
  it("nudges $team prompt-submit routing toward omxc team runtime usage", async () => {
    ...
    try {
      ...
      assert.match(JSON.stringify(result.outputJson), /Use the durable OMX team runtime via `omxc team \.\.\.`/);
      assert.match(JSON.stringify(result.outputJson), /If you need help, run `omxc team --help`\./);
