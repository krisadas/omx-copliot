 ...
 *
 * Usage:
 *   omxc hud              Show current HUD state
 *   omxc hud --watch      Poll every 1s with terminal clear
 *   omxc hud --json       Output raw state as JSON
 *   omxc hud --preset=X   Use preset: minimal, focused, full
 *   omxc hud --tmux       Open HUD in a tmux split pane (auto-detects orientation)
 */
...
async function launchTmuxPane(cwd: string, flags: HudFlags): Promise<void> {
  ...
  if (!process.env.TMUX) {
    console.error('Not inside a tmux session. Start tmux first, then run: omxc hud --tmux');
    process.exit(1);
  }
  ...
  try {
    // Split bottom pane, 4 lines tall, running omxc hud --watch.
