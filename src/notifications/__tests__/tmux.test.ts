...
describe('getTeamTmuxSessions - session matching', () => {
  ...
  it('returns canonical session name (omxc-team-alpha) for team "alpha"', () => {
    makeFakeTmux(['omxc-team-alpha', 'other-session']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omxc-team-alpha']);
  });

  it('returns prefixed worker sessions (omxc-team-alpha-worker1)', () => {
    makeFakeTmux(['omxc-team-alpha-worker1', 'omxc-team-alpha-worker2']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omxc-team-alpha-worker1', 'omxc-team-alpha-worker2']);
  });

  it('does NOT return sessions for a different team', () => {
    makeFakeTmux(['omxc-team-beta', 'omxc-team-beta-worker1']);
