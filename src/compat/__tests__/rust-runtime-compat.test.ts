  ...
  it('keeps team status on the manifest-authored compatibility view', async () => {
    ...
    try {
      ...
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        config.tmux_session = 'omxc-team-legacy-rust-compat-team';
        manifest.workspace_mode = 'worktree';
        manifest.tmux_session = 'omxc-team-rust-compat-team';
  ...
  it('keeps doctor --team on the manifest-authored tmux session', async () => {
    ...
    try {
      ...
      await withTempTeamStateRoot(teamStateRoot, async () => {
        ...
        config.tmux_session = 'omxc-team-legacy-rust-compat-doctor';
        manifest.tmux_session = 'omxc-team-rust-compat-doctor';
        ...
        await writeFile(
          tmuxPath,
          '#!/bin/sh\nif [ "$1" = "list-sessions" ]; then echo "omxc-team-rust-compat-doctor"; exit 0; fi\nexit 0\n',
        );
