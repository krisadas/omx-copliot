...
describe('buildPlatformCommandSpec', () => {
  it('wraps .cmd shims through ComSpec on Windows', async () => {
    ...
    try {
      const cmdPath = join(fakeBin, 'copilot-cli.cmd');
      ...
      assert.match(spec.args[3] || '', /copilot-cli\.cmd/i);
  ...
  it('prefers the npm-installed copilot-cli.js entrypoint over Windows shims when available', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omx-platform-ps1-'));
    try {
      const ps1Path = join(fakeBin, 'copilot-cli.ps1');
      const cmdPath = join(fakeBin, 'copilot-cli.cmd');
      const codexJsPath = join(fakeBin, 'node_modules', '@openai', 'copilot-cli', 'bin', 'copilot-cli.js');
      await writeFile(ps1Path, '');
      await writeFile(cmdPath, '');
      await mkdir(join(fakeBin, 'node_modules', '@openai', 'copilot-cli', 'bin'), { recursive: true });
  ...
  it('resolves copilot-cli.js from npm .bin-style Windows shim layouts', async () => {
    const fakeRoot = await mkdtemp(join(tmpdir(), 'omx-platform-local-shim-'));
    const fakeBin = join(fakeRoot, 'node_modules', '.bin');
    try {
      const cmdPath = join(fakeBin, 'copilot-cli.cmd');
      const codexJsPath = join(fakeRoot, 'node_modules', '@openai', 'copilot-cli', 'bin', 'copilot-cli.js');
      await mkdir(fakeBin, { recursive: true });
      await mkdir(join(fakeRoot, 'node_modules', '@openai', 'copilot-cli', 'bin'), { recursive: true });
      await writeFile(cmdPath, '@echo off\r\n');
      await writeFile(codexJsPath, '');
      const spec = buildPlatformCommandSpec(
        'copilot-cli',
  ...
  it('skips directory PATH candidates when resolving the PowerShell executable for .ps1 shims', async () => {
    ...
    try {
      const ps1Path = join(shimBin, 'copilot-cli.ps1');
...
describe('spawnPlatformCommandSync', () => {
  it('passes the Windows-resolved spec into the spawn implementation', async () => {
    ...
    try {
      ...
      assert.match((calls[0]?.args[3] || ''), /copilot-cli\.cmd/i);
  ...
  it('launches Windows cmd shims successfully with the real spawn implementation', async () => {
    ...
    try {
      ...
      assert.equal((probed.result.stdout || '').trim(), 'fake-copilot-cli 1.2.3');
    } finally {
      await rm(fakeBin, { recursive: true, force: true });
    }
  });
