...
const HELP = `
Usage:
  omxc tmux-hook init       Create .omx/tmux-hook.json
  omxc tmux-hook status     Show config + runtime state summary
  omxc tmux-hook validate   Validate config and tmux target reachability
  omxc tmux-hook test       Run a synthetic notify-hook turn (end-to-end)
`;
...
async function readValidatedConfig(cwd = process.cwd()): Promise<TmuxHookConfig> {
  ...
  if (!existsSync(configPath)) {
    throw new Error('tmux-hook config missing. Run: omxc tmux-hook init');
  }
...
async function testTmuxHook(args: string[]): Promise<void> {
  ...
  const payload = {
    ...
    'thread-id': threadId,
    'turn-id': turnId,
    'input-messages': ['omxc tmux-hook test'],
