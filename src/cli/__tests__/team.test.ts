...
describe('parseTeamStartArgs', () => {
  ...
  it('rejects deprecated omxc team ralph syntax', () => {
    assert.throws(
      () => parseTeamStartArgs(['ralph', '--worktree=feature/demo', '4:executor', 'ship', 'it']),
      /Deprecated usage: `omxc team ralph \.\.\.` has been removed/,
    );
  });
  ...
  it('reuses the approved team launch hint for a short English follow-up', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-followup-en-'));
    ...
    try {
      ...
      await writeFile(
        ...
        '# Approved plan\n\nLaunch via omxc team 3:executor "Execute approved issue 831 plan"\n',
      );
  ...
  it('reuses the approved team launch hint for a short Korean follow-up', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-followup-ko-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omx', 'plans'), { recursive: true });
  ...
  it('preserves explicit team staffing overrides while reusing the approved plan task', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-followup-override-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omx', 'plans'), { recursive: true });
...
describe('teamCommand api', () => {
  it('builds leader monitoring hints that keep team status visible while ON', () => {
    ...
    assert.equal(hints[0], 'leader_check: omxc team status my-team');
    assert.match(hints[1] ?? '', /while ON, keep checking state/);
    assert.match(hints[1] ?? '', /sleep 30 && omxc team status my-team/);
  });

  it('prints team-specific help for omxc team --help', async () => {
    const logs: string[] = [];
    ...
    try {
      ...
      assert.match(logs[0] ?? '', /Usage: omxc team \[N:agent-type\]/);
      assert.match(logs[0] ?? '', /omxc team api <operation>/);
  ...
  it('prints team-specific help for omxc team help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
  ...
  it('prints team-api-specific help for omxc team api --help', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omxc team api <operation>/);
  ...
  it('prints team-api-specific help for omxc team api help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
  ...
  it('prints operation-specific help for omxc team api <operation> --help', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omxc team api send-message --input <json> \[--json\]/);
  ...
  it('prints operation-specific help for omxc team api <operation> help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omxc team api claim-task --input <json> \[--json\]/);
  ...
  it('prints event query help for omxc team api read-events help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omxc team api read-events --input <json> \[--json\]/);
  ...
  it('executes read-events via CLI api with canonical JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-api-read-events-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omxc team api read-events');
  ...
  it('executes read-idle-state via CLI api with structured JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-api-read-idle-state-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omxc team api read-idle-state');
      assert.equal(envelope.ok, true);
  ...
  it('executes read-stall-state via CLI api with structured JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-api-read-stall-state-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omxc team api read-stall-state');
      assert.equal(envelope.ok, true);
  ...
  it('executes CLI interop operation with stable JSON envelope', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-api-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omxc team api send-message');
      assert.equal(envelope.ok, true);
  ...
  it('returns deterministic JSON errors for invalid api usage with --json', async () => {
    ...
    try {
      ...
      assert.equal(envelope.schema_version, '1.0');
      assert.equal(typeof envelope.timestamp, 'string');
      assert.equal(envelope.command, 'omxc team api');
      assert.equal(envelope.ok, false);
      assert.equal(envelope.operation, 'unknown');
      assert.equal(envelope.error?.code, 'invalid_input');
      assert.match(envelope.error?.message ?? '', /Usage: omxc team api/);
  ...
  it('supports claim-safe lifecycle via CLI api (create -> claim -> transition)', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-api-lifecycle-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('accepts new canonical event types via CLI api append-event', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-api-event-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
...
describe('teamCommand status', () => {
  it('prints pane ids and sparkshell hint when tmux panes are recorded', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-status-panes-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      config.workers[0]!.worker_cli = 'copilot-cli';
      ...
      assert.match(output, /sparkshell_hint: omxc sparkshell --tmux-pane <pane-id> --tail-lines 400/);
      assert.match(output, /inspect_leader: omxc sparkshell --tmux-pane %10 --tail-lines 400/);
      assert.match(output, /inspect_hud: omxc sparkshell --tmux-pane %11 --tail-lines 400/);
      assert.match(output, /inspect_worker-1: omxc sparkshell --tmux-pane %21 --tail-lines 400/);
      assert.match(output, /inspect_worker-2: omxc sparkshell --tmux-pane %22 --tail-lines 400/);
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
  ...
  it('returns pane ids and sparkshell hint in JSON mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-status-json-'));
    const wdPath = await realpath(wd);
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(payload.command, 'omxc team status');
      ...
      const taskPath = payload.panes?.recommended_inspect_task_paths?.['worker-1'];
      assert.equal(typeof taskPath, 'string');
      assert.match(taskPath as string, /\.omx\/state\/team\/pane-json-team\/tasks\/task-1\.json$/);
      assert.deepEqual(payload.panes?.recommended_inspect_approval_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/approvals/task-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_state_dirs, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_status_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/status.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_heartbeat_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/heartbeat.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_identity_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/identity.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_inbox_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/inbox.md` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_mailbox_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/mailbox/worker-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_shutdown_request_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/shutdown-request.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_shutdown_ack_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/shutdown-ack.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_dir_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_config_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/config.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_manifest_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/manifest.v2.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_events_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/events/events.ndjson` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_dispatch_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/dispatch/requests.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_phase_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/phase.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_monitor_snapshot_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/monitor-snapshot.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_summary_snapshot_paths, { 'worker-1': `${wdPath}/.omx/state/team/pane-json-team/summary-snapshot.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_panes, { 'worker-1': '%41' });
      assert.equal(payload.panes?.recommended_inspect_command, 'omxc sparkshell --tmux-pane %41 --tail-lines 400');
      assert.deepEqual(payload.panes?.recommended_inspect_commands, ['omxc sparkshell --tmux-pane %41 --tail-lines 400']);
      assert.equal(payload.panes?.recommended_inspect_summary, 'target=worker-1 pane=%41 cli=claude role=executor alive=false turn_count=5 turns_without_progress=0 reason=dead_worker state=working task=1 subject=Recover worker-1 progress command=omxc sparkshell --tmux-pane %41 --tail-lines 400');
      assert.deepEqual(payload.panes?.recommended_inspect_items, [{
        ...
        task_claim_lock_path: `${wdPath}/.omx/state/team/pane-json-team/claims/task-1.lock`,
        ...
        task_path: `${wdPath}/.omx/state/team/pane-json-team/tasks/task-1.json`,
        approval_path: `${wdPath}/.omx/state/team/pane-json-team/approvals/task-1.json`,
        worker_state_dir: `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1`,
        worker_status_path: `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/status.json`,
        worker_heartbeat_path: `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/heartbeat.json`,
        worker_identity_path: `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/identity.json`,
        worker_inbox_path: `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/inbox.md`,
        worker_mailbox_path: `${wdPath}/.omx/state/team/pane-json-team/mailbox/worker-1.json`,
        worker_shutdown_request_path: `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/shutdown-request.json`,
        worker_shutdown_ack_path: `${wdPath}/.omx/state/team/pane-json-team/workers/worker-1/shutdown-ack.json`,
        team_dir_path: `${wdPath}/.omx/state/team/pane-json-team`,
        team_config_path: `${wdPath}/.omx/state/team/pane-json-team/config.json`,
        team_manifest_path: `${wdPath}/.omx/state/team/pane-json-team/manifest.v2.json`,
        team_events_path: `${wdPath}/.omx/state/team/pane-json-team/events/events.ndjson`,
        team_dispatch_path: `${wdPath}/.omx/state/team/pane-json-team/dispatch/requests.json`,
        team_phase_path: `${wdPath}/.omx/state/team/pane-json-team/phase.json`,
        team_monitor_snapshot_path: `${wdPath}/.omx/state/team/pane-json-team/monitor-snapshot.json`,
        team_summary_snapshot_path: `${wdPath}/.omx/state/team/pane-json-team/summary-snapshot.json`,
        command: 'omxc sparkshell --tmux-pane %41 --tail-lines 400',
      }]);
      ...
      assert.equal(payload.panes?.sparkshell_hint, 'omxc sparkshell --tmux-pane <pane-id> --tail-lines 400');
      assert.deepEqual(payload.panes?.sparkshell_commands, {
        leader: 'omxc sparkshell --tmux-pane %30 --tail-lines 400',
        hud: 'omxc sparkshell --tmux-pane %31 --tail-lines 400',
        'worker-1': 'omxc sparkshell --tmux-pane %41 --tail-lines 400',
      });
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
  ...
  it('prints workspace_mode in text status output when present', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-status-workspace-mode-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('returns workspace_mode in JSON status output when present', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-status-json-workspace-mode-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('returns a missing envelope in JSON mode when team state is absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-status-missing-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('records leader runtime activity when team status is read', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-status-activity-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('supports custom tail lines for generated sparkshell commands', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-status-tail-lines-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.match(logs.join('\n'), /inspect_worker-1: omxc sparkshell --tmux-pane %51 --tail-lines 600/);
      ...
      assert.equal(payload.panes?.sparkshell_commands?.['worker-1'], 'omxc sparkshell --tmux-pane %51 --tail-lines 550');
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
...
describe('teamCommand await', () => {
  it('returns next canonical event for a team in JSON mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-await-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('returns a dead-worker event for the prompt-launch smoke path instead of timing out', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-await-prompt-dead-'));
    const binDir = join(wd, 'bin');
    const fakeCodexPath = join(binDir, 'copilot-cli');
    const previousCwd = process.cwd();
...
`,
    ...
    try {
      ...
      process.env.OMX_TEAM_WORKER_CLI = 'copilot-cli';
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.ok(payload.status === 'event' || payload.status === 'timeout');
  ...
  it('initializes and rehydrates active team mode state on start and resume', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-mode-state-'));
    const binDir = join(wd, 'bin');
    const fakeCodexPath = join(binDir, 'copilot-cli');
    const previousCwd = process.cwd();
    const previousPath = process.env.PATH;
...
`,
  ...
  it('does not resurrect active team mode state when canonical team phase is terminal on resume', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omxc-team-mode-terminal-'));
    const binDir = join(wd, 'bin');
    const fakeCodexPath = join(binDir, 'copilot-cli');
    const previousCwd = process.cwd();
    const previousPath = process.env.PATH;
...
`,
  ...
  it('rejects legacy omxc team ralph launches at command entry', async () => {
