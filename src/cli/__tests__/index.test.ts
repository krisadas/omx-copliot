...
describe("cleanupLaunchOrphanedMcpProcesses", () => {
  it("reaps only detached OMX MCP processes without a live Codex ancestor", async () => {
    const processes: ProcessEntry[] = [
      { pid: 700, ppid: 500, command: "copilot-cli" },
      ...
      },
      {
        pid: 820,
        ppid: 50,
        command: "copilot-cli --model gpt-5",
      },
      {
        pid: 821,
        ppid: 820,
...
describe("reapStaleNotifyFallbackWatcher", () => {
  ...
  it("suppresses ESRCH cleanup errors but warns on unexpected failures", async () => {
    ...
    try {
      ...
      assert.equal(warned[0]?.message, "[omxc] warning: failed to stop stale notify fallback watcher");
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("findHudWatchPaneIds detects stale HUD watch panes and excludes current pane", () => {
    const panes = parseTmuxPaneSnapshot(
      [
        ...
        "%4\tcodex\tcopilot-cli --model gpt-5",
      ].join("\n"),
    );
...
describe("detached tmux new-session sequencing", () => {
  ...
  it("buildDetachedSessionBootstrapSteps starts native Windows detached sessions with powershell", () => {
    ...
    const steps = buildDetachedSessionBootstrapSteps(
      "omx-demo",
      "C:/project",
      "'copilot-cli' '--dangerously-bypass-approvals-and-sandbox'",
      hudCmd,
      "--model gpt-5",
      "C:/codex-home",
      null,
  ...
  it("detached leader command executes codex and cleanup without shell-quote breakage", async () => {
    ...
    try {
      ...
      await writeFile(
        join(fakeBin, "copilot-cli"),
        `#!/bin/sh
printf 'codex:%s\\n' "$*" >> "${logPath}"
exit 0
`,
      );
      await chmod(join(fakeBin, "copilot-cli"), 0o755);
      await writeFile(
        join(fakeBin, "tmux"),
        `#!/bin/sh
printf 'tmux:%s\\n' "$*" >> "${logPath}"
...
describe("buildTmuxShellCommand", () => {
  it("preserves quoted config values for tmux shell-command execution", () => {
    assert.equal(
      buildTmuxShellCommand("copilot-cli", [
        "--dangerously-bypass-approvals-and-sandbox",
        "-c",
        'model_reasoning_effort="xhigh"',
      ]),
      `'copilot-cli' '--dangerously-bypass-approvals-and-sandbox' '-c' 'model_reasoning_effort="xhigh"'`,
    );
  });
});
...
describe("buildTmuxPaneCommand", () => {
  ...
  it("wraps command with bash profile sourcing for bash shell", () => {
    const result = buildTmuxPaneCommand("copilot-cli", [], "/bin/bash");
    assert.ok(
  ...
  it("skips rc sourcing for unknown shells but still uses login flag", () => {
    const result = buildTmuxPaneCommand("copilot-cli", [], "/bin/fish");
    assert.ok(
  ...
  it("falls back to /bin/sh when shell path is empty", () => {
    const result = buildTmuxPaneCommand("copilot-cli", [], "");
    assert.ok(
...
describe("buildWindowsPromptCommand", () => {
  it("encodes detached Windows commands for safe PowerShell prompt injection", () => {
    const result = buildWindowsPromptCommand("copilot-cli", [
      "--dangerously-bypass-approvals-and-sandbox",
      "-c",
      'model_reasoning_effort="high"',
      "it's",
    ...
    assert.equal(
      decoded,
      "$ErrorActionPreference = 'Stop'; & { & 'copilot-cli' '--dangerously-bypass-approvals-and-sandbox' '-c' 'model_reasoning_effort=\"high\"' 'it''s' }",
    );
  });
});
