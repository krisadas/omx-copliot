  ...
  it('ignores non-compatibility stderr failures', () => {
    assert.equal(
      isSparkShellNativeCompatibilityFailure({
        ...
        output: [],
        stdout: '',
        stderr: 'omxc sparkshell: summary unavailable (tmux failed)\n',
        status: 1,
        signal: null,
      }),
      false,
...
describe('omxc sparkshell', () => {
  it('includes sparkshell in top-level help output', async () => {
    ...
    try {
      ...
      assert.match(result.stdout, /omxc explore\s+Default read-only exploration entrypoint \(may adaptively use sparkshell backend\)/);
      assert.match(result.stdout, /omxc sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /omxc sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
  ...
  it('prints sparkshell usage when invoked with --help', async () => {
    ...
    try {
      ...
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.match(result.stdout, /Usage: omxc sparkshell <command> \[args\.\.\.\]/);
      assert.match(result.stdout, /or: omxc sparkshell --tmux-pane <pane-id> \[--tail-lines <100-1000>\]/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
