...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omxc-autoresearch-guided-test-'));
...
describe('parseInitArgs', () => {
  ...
  it('sanitizes slug via slugifyMissionName', () => {
    const result = parseInitArgs(['--slug', '../../etc/cron.d/omxc']);
...
describe('checkTmuxAvailable', () => {
  ...
  it('launches background tmux sessions with an absolute omxc entry path even from a relative launcher', async () => {
    const repo = await initRepo();
    const fakeBin = await mkdtemp(join(tmpdir(), 'omxc-autoresearch-guided-bin-'));
    const startupCwd = await mkdtemp(join(tmpdir(), 'omxc-autoresearch-guided-start-'));
...
`,
      ...
      assert.match(tmuxOutput, /new-session -d -s omxc-autoresearch-demo/);
      assert.match(tmuxOutput, new RegExp(escapeRegExp(launcherPath)));
      assert.doesNotMatch(tmuxOutput, /dist\/cli\/omxc\.js autoresearch/);
    } finally {
