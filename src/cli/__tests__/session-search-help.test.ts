...
describe('omxc session help', () => {
  it('documents the session search command in help output', async () => {
    ...
    try {
      ...
      assert.match(mainHelp.stdout, /omxc resume\s+Resume a previous interactive Codex session/i);
      assert.match(mainHelp.stdout, /omxc autoresearch\s+Launch thin-supervisor autoresearch with keep\/discard\/reset parity/i);
      assert.match(mainHelp.stdout, /omxc session\s+Search prior local session transcripts/i);

      const sessionHelp = runOmx(cwd, ['session', '--help']);
      assert.equal(sessionHelp.status, 0, sessionHelp.stderr || sessionHelp.stdout);
      assert.match(sessionHelp.stdout, /omxc session search <query>/i);
