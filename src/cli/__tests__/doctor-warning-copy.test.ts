...
describe('omxc doctor onboarding warning copy', () => {
...
`.trimStart(),
      ...
      assert.match(
        res.stdout,
        /Config: config\.toml exists but no OMX entries yet \(expected before first setup; run "omxc setup --force" once\)/,
      );
      assert.match(
        res.stdout,
        /MCP Servers: 1 servers but no OMX servers yet \(expected before first setup; run "omxc setup --force" once\)/,
      );
    } finally {
  ...
  it('warns when explore harness sources are packaged but cargo is unavailable', async () => {
    ...
    try {
      await withPackagedExploreHarnessHidden(async () => {
        ...
        await writeFile(join(fakeBin, 'copilot-cli'), '#!/bin/sh\necho "codex test"\n');
        spawnSync('chmod', ['+x', join(fakeBin, 'copilot-cli')], { encoding: 'utf-8' });
        ...
        assert.match(
          res.stdout,
          /Explore Harness: (Rust harness sources are packaged, but no compatible packaged prebuilt or cargo was found \(install Rust or set OMX_EXPLORE_BIN for omxc explore\)|not ready \(no packaged binary, OMX_EXPLORE_BIN, or cargo toolchain\))/,
        );
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
