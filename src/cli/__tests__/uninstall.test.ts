...
describe('omxc uninstall', () => {
  ...
  it('warns when overlapping legacy ~/.agents/skills remains after user-scope uninstall', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Warning: 1 overlapping skill names remain between .*\.codex[\\/]+skills and .*\.agents[\\/]+skills; 1 differ in SKILL\.md content\. omxc uninstall only removes the active canonical skill root; archive or remove ~\/\.agents\/skills if Codex still shows duplicates/,
      );
  ...
  it('warns when a distinct legacy ~/.agents/skills root remains after user-scope uninstall', async () => {
    ...
    try {
      ...
      assert.match(
        res.stdout,
        /Warning: legacy ~\/\.agents\/skills still exists \(1 skills\)\. omxc uninstall does not remove that historical root automatically; archive or remove ~\/\.agents\/skills if Codex still shows stale or duplicate skills/,
      );
      assert.equal(existsSync(canonicalHelp), false, 'canonical OMX skill should be removed');
      assert.equal(existsSync(join(home, '.agents', 'skills')), true, 'legacy skill root should remain for manual cleanup');
    } finally {
  ...
  it('does not warn about legacy ~/.agents/skills when none exists', async () => {
    ...
    try {
      ...
      assert.doesNotMatch(res.stdout, /omxc uninstall does not remove legacy ~\/\.agents\/skills/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
