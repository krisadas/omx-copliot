...
describe('package bin contract', () => {
  it('declares omxc with an explicit relative bin path and avoids packaging platform-specific native binaries', () => {
    ...
    assert.deepEqual(pkg.bin, { omxc: 'dist/cli/omx.js' });
    ...
    for (const scriptName of ['test:node', 'coverage:team-critical', 'coverage:ts:full', 'test:compat:node'] as const) {
