...
describe('exploreCommand', () => {
  ...
  it('works end-to-end through omxc explore', async () => {
  ...
  it('launches an env-node codex binary while keeping model shell commands allowlisted', async () => {
    ...
    try {
      await withPackagedExploreHarnessHidden(async () => {
        ...
        assert.match(captured, /--BLOCKED_STDERR--[\s\S]*not on the omxc explore allowlist/);
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
    ...
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '%s\n' '# Answer' '## Critical facts' '- MUST: summary mode stayed read-only' '- MUST: blocked command stayed node --version' '- MUST: next command is omxc team status <team-name>' '' '## Noise'
i=0
while [ "$i" -lt 80 ]; do
  printf '%s\n' "- distractor line $i"
  i=$((i + 1))
...
`,
        ...
        assert.match(result.stdout, /MUST: next command is omxc team status <team-name>/);
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
    ...
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '[omxc explore] spark model \`%s\` unavailable or failed (exit 17). Falling back to \`gpt-5.4\`.\n' "\${OMX_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
printf '[omxc explore] spark stderr: spark timed out; retry with the frontier fallback\n' >&2
    ...
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '[omxc explore] spark model \`%s\` unavailable or failed (exit 23). Falling back to \`gpt-5.4\`.\n' "\${OMX_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
printf '[omxc explore] spark stderr: spark backend unavailable; install the fallback runtime\n' >&2
printf '[omxc explore] both spark (\`%s\`) and fallback (\`gpt-5.4\`) attempts failed (codes 23 / 29). Last stderr: fallback backend unavailable; set OMX_EXPLORE_BIN to a working harness\n' "\${OMX_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
exit 1
`,
        );
