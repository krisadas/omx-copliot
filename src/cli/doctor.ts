...
export async function doctor(options: DoctorOptions = {}): Promise<void> {
  ...
  if (failCount > 0) {
    console.log('\nRun "omxc setup" to fix installation issues.');
  } else if (warnCount > 0) {
...
async function doctorTeam(): Promise<void> {
  ...
  // Ensure non-zero exit for `omxc doctor --team` failures.
...
async function collectTeamDoctorIssues(cwd: string): Promise<TeamDoctorIssue[]> {
  ...
  for (const teamName of teamDirs) {
    ...
    let tmuxSession = `omxc-team-${teamName}`;
  ...
  if ((existsSync(hudStatePath) || existsSync(leaderActivityPath)) && teamDirs.length > 0) {
    try {
      ...
      if (leaderIsStale && !tmuxUnavailable) {
        ...
        for (const teamName of teamDirs) {
          const session = knownTeamSessions.has(`omxc-team-${teamName}`)
            ? `omxc-team-${teamName}`
...
function listTeamTmuxSessions(): Set<string> | null {
  ...
  const sessions = (res.stdout || '')
    .split('\n')
    .map((s) => s.trim())
    .filter((s) => s.startsWith('omxc-team-'));
...
function checkExploreHarness(): Check {
  ...
  if (!existsSync(manifestPath)) {
    return {
      ...
      status: 'warn',
      message: 'Rust harness sources not found in this install (omxc explore unavailable until packaged or OMX_EXPLORE_BIN is set)',
    };
  }
  ...
  if (result.error) {
    ...
    if (kind === 'missing') {
      return {
        name: 'Explore Harness',
        status: 'warn',
        message: `Rust harness sources are packaged, but no compatible packaged prebuilt or cargo was found (install Rust or set ${EXPLORE_BIN_ENV} for omxc explore)`,
      };
    }
    return {
      name: 'Explore Harness',
...
async function checkConfig(configPath: string): Promise<Check> {
  ...
  try {
    ...
    return {
      name: 'Config',
      status: 'warn',
      message: 'config.toml exists but no OMX entries yet (expected before first setup; run "omxc setup --force" once)',
    };
  } catch {
...
function checkAgentsMd(scope: DoctorSetupScope, codexHomeDir: string): Check {
  if (scope === 'user') {
    ...
    return {
      name: 'AGENTS.md',
      status: 'warn',
      message: `not found in ${userAgentsMd} (run omxc setup --scope user)`,
    };
  }
  ...
  return {
    name: 'AGENTS.md',
    status: 'warn',
    message: 'not found in project root (run omxc agents-init . or omxc setup --scope project)',
  };
}
  ...
  try {
    ...
    if (mcpCount > 0) {
      ...
      return {
        name: 'MCP Servers',
        status: 'warn',
        message: `${mcpCount} servers but no OMX servers yet (expected before first setup; run "omxc setup --force" once)`,
      };
    }
