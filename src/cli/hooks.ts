...
const HELP = `
Usage:
  omxc hooks init       Create .omx/hooks/sample-plugin.mjs scaffold
  omxc hooks status     Show plugin directory + discovered plugins
  omxc hooks validate   Validate plugin exports/signatures
  omxc hooks test       Dispatch synthetic turn-complete event to plugins

Notes:
  - This command is additive. Existing \`omxc tmux-hook\` behavior is unchanged.
...
async function validateHooks(): Promise<void> {
  ...
  const plugins = await discoverHookPlugins(cwd);
  if (plugins.length === 0) {
    console.log('No plugins found. Run: omxc hooks init');
    return;
  }
