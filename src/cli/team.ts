...
const TEAM_HELP = `
Usage: omxc team [N:agent-type] "<task description>"
       omxc team status <team-name> [--json] [--tail-lines <100-1000>]
       omxc team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]
       omxc team resume <team-name>
       omxc team shutdown <team-name> [--force]
       omxc team api <operation> [--input <json>] [--json]
       omxc team api --help

Notes:
  team workers use dedicated worktrees automatically by default.
  --worktree is deprecated for omxc team and is now only a backward-compatible no-op override.
  use native Codex subagents for small in-session fanout; use omxc team for durable tmux/state/worktree coordination.

Examples:
  omxc team 3:executor "fix failing tests"
  omxc team status my-team
  omxc team status my-team --json
  omxc team status my-team --tail-lines 600
  omxc team api send-message --input '{"team_name":"my-team","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK"}' --json
`;

const TEAM_API_HELP = `
Usage: omxc team api <operation> [--input <json>] [--json]
       omxc team api <operation> --help

Supported operations:
  ${TEAM_API_OPERATIONS.join('\n  ')}

Examples:
  omxc team api list-tasks --input '{"team_name":"my-team"}' --json
  omxc team api claim-task --input '{"team_name":"my-team","task_id":"1","worker":"worker-1","expected_version":1}' --json
`;
...
const TEAM_API_OPERATION_NOTES: Partial<Record<TeamApiOperation, string>> = {
  ...
  'read-events': 'Events are returned in canonical form; worker_idle log entries normalize to type worker_state_changed with source_type worker_idle. wakeable_only defaults to false; set wakeable_only=true to mirror omxc team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).',
  'await-event': 'Waits for the next matching event and returns status=timeout when no matching event arrives before timeout_ms. wakeable_only defaults to false; set wakeable_only=true to mirror omxc team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).',
...
${required}${optional}${note}Example:
  omxc team api ${operation} --input '${sampleInputJson}' --json
`.trim();
}
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (token === '--tail-lines') {
      ...
      if (!Number.isFinite(parsed) || parsed < MIN_SPARKSHELL_TAIL_LINES || parsed > MAX_SPARKSHELL_TAIL_LINES) {
        throw new Error(`Usage: omxc team status <team-name> [--json] [--tail-lines <${MIN_SPARKSHELL_TAIL_LINES}-${MAX_SPARKSHELL_TAIL_LINES}>]`);
      }
      return parsed;
    }
...
function parseTeamApiArgs(args: string[]): {
  ...
  if (!operation) {
    throw new Error(`Usage: omxc team api <operation> [--input <json>] [--json]\nSupported operations: ${TEAM_API_OPERATIONS.join(', ')}`);
  }
  ...
  for (let i = 1; i < args.length; i += 1) {
    ...
    throw new Error(`Unknown argument for "omxc team api": ${token}`);
  }
...
function renderTeamPaneStatus(
  ...
  if (paneStatus.sparkshell_hint) {
    console.log('sparkshell_hint: omxc sparkshell --tmux-pane <pane-id> --tail-lines 400');
  }
...
function parseTeamArgs(args: string[], cwd: string = process.cwd()): ParsedTeamArgs {
  ...
  if (tokens[0]?.toLowerCase() === 'ralph') {
    throw new Error('Deprecated usage: `omxc team ralph ...` has been removed. Use `omxc team ...` or run `omxc ralph ...` separately.');
  }
  ...
  if (!task) {
    throw new Error('Usage: omxc team [N:agent-type] "<task description>"');
  }
...
export function buildLeaderMonitoringHints(teamName: string): string[] {
  ...
  return [
    `leader_check: omxc team status ${sanitized}`,
    `leader_loop_hint: while ON, keep checking state (example: sleep 30 && omxc team status ${sanitized})`,
  ];
}
...
export async function teamCommand(args: string[], _options: TeamCliOptions = {}): Promise<void> {
  ...
  if (subcommand === 'api') {
    ...
    if (parsedApi.json) {
      console.log(JSON.stringify({
        ...jsonBase,
        command: `omxc team api ${parsedApi.operation}`,
        ...envelope,
      }));
  ...
  if (subcommand === 'status') {
    ...
    const wantsJson = teamArgs.includes('--json');
    if (!name) throw new Error('Usage: omxc team status <team-name> [--json]');
  ...
  if (subcommand === 'await') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omxc team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]');
    const wantsJson = teamArgs.includes('--json');
  ...
  if (subcommand === 'resume') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omxc team resume <team-name>');
  ...
  if (subcommand === 'shutdown') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omxc team shutdown <team-name> [--force]');
    ...
    await updateModeState('team', {
      ...
      console.warn('[omxc] warning: failed to persist team mode shutdown state', {
        team: name,
