...
export async function maybePromptGithubStar(deps: MaybePromptGithubStarDeps = {}): Promise<void> {
  ...
  const approved = await askYesNoImpl('[omxc] Enjoying oh-my-codex? Star it on GitHub? [Y/n] ');
  if (!approved) return;
  ...
  if (star.ok) {
    ...
    log('[omxc] Thanks for the star!');
    return;
  }
  const warn = deps.warnFn ?? console.warn;
  warn(`[omxc] Could not star repository automatically: ${star.error}`);
}
