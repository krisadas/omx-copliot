...
const HELP = `omxc session - Search prior local session history

Usage:
  omxc session search <query> [options]
...
Examples:
  omxc session search "worker inbox path"
  omxc session search all_workers_idle --since 7d --limit 5
  omxc session search "team api" --project current --json
`;
