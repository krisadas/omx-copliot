...
export const EXPLORE_USAGE = [
  'Usage: omxc explore --prompt "<prompt>"',
  '   or: omxc explore --prompt-file <file>',
].join('\n');
...
export async function exploreCommand(args: string[]): Promise<void> {
  ...
  if (sparkShellRoute) {
    try {
      ...
      process.stderr.write(`[omxc explore] sparkshell backend unavailable (${message}). Falling back to the explore harness.\n`);
    }
  }
