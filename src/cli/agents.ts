...
const AGENTS_USAGE = [
  'Usage:',
  '  omxc agents list [--scope user|project]',
  '  omxc agents add <name> [--scope user|project] [--force]',
...
export async function agentsCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      const name = args[1];
      assert.ok(name, 'Usage: omxc agents add <name>');
    ...
    case 'edit': {
      const name = args[1];
      assert.ok(name, 'Usage: omxc agents edit <name>');
    ...
    case 'remove': {
      const name = args[1];
      assert.ok(name, 'Usage: omxc agents remove <name>');
