...
export const AGENTS_INIT_USAGE = [
  "Usage: omxc agents-init [path] [--dry-run] [--force] [--verbose]",
  "       omxc deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
...
export async function agentsInit(
  ...
  console.log("omx-copilot AGENTS bootstrap");
  console.log("===========================\n");
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active omxc session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by omxc agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active omxc session is running.\n",
  );
