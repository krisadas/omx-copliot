...
export const ASK_USAGE = [
  ...
  '   or: omxc ask <claude|gemini> -p "<prompt>"',
  '   or: omxc ask claude --print "<prompt>"',
  '   or: omxc ask gemini --prompt "<prompt>"',
  '   or: omxc ask <claude|gemini> --agent-prompt <role> "<prompt>"',
  '   or: omxc ask <claude|gemini> --agent-prompt=<role> --prompt "<prompt>"',
].join('\n');
...
async function resolveAgentPromptContent(
  ...
  if (!existsSync(promptsDir)) {
    throw new Error(`[ask] prompts directory not found: ${promptsDir}. Run "omxc setup" to install prompts.`);
  }
