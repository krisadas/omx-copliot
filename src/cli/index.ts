...
const HELP = `
oh-my-codex (omxc) - Multi-agent orchestration for Copilot CLI

Usage:
  omxc          Launch Copilot CLI (HUD auto-attaches only when already inside tmux)
  omxc exec      Run copilot-cli exec non-interactively with OMX AGENTS/overlay injection
  omxc setup     Install skills, prompts, MCP servers, and scope-specific AGENTS.md
  omxc uninstall Remove OMX configuration and clean up installed artifacts
  omxc doctor    Check installation health
  omxc cleanup   Kill orphaned OMX MCP server processes and remove stale OMX /tmp directories
  omxc doctor --team  Check team/swarm runtime health diagnostics
  omxc ask       Ask local provider CLI (claude|gemini) and write artifact output
  omxc resume    Resume a previous interactive Copilot CLI session
  omxc explore   Default read-only exploration entrypoint (may adaptively use sparkshell backend)
  omxc session   Search prior local session transcripts and history artifacts
  omxc agents-init [path]
                Bootstrap lightweight AGENTS.md files for a repo/subtree
  omxc agents    Manage Codex native agent TOML files
  omxc deepinit [path]
                Alias for agents-init (lightweight AGENTS bootstrap only)
  omxc team      Spawn parallel worker panes in tmux and bootstrap inbox/task state
  omxc ralph     Launch Copilot CLI with ralph persistence mode active
  omxc autoresearch Launch thin-supervisor autoresearch with keep/discard/reset parity
  omxc version   Show version information
  omxc tmux-hook Manage tmux prompt injection workaround (init|status|validate|test)
  omxc hooks     Manage hook plugins (init|status|validate|test)
  omxc hud       Show HUD statusline (--watch, --json, --preset=NAME)
  omxc sparkshell <command> [args...]
  omxc sparkshell --tmux-pane <pane-id> [--tail-lines <100-1000>]
  ...
  omxc help      Show this help message
  omxc status    Show active modes and state
  omxc cancel    Cancel active execution modes
  omxc reasoning Show or set model reasoning effort (low|medium|high|xhigh)
...
Options:
  ...
  -w, --worktree[=<name>]
                Launch Copilot CLI in a git worktree (detached when no name is given)
  --force       Force reinstall (overwrite existing files)
  --dry-run     Show what would be done without doing it
  --keep-config Skip config.toml cleanup during uninstall
  --purge       Remove .omx/ cache directory during uninstall
  --verbose     Show detailed output
  --scope       Setup scope for "omxc setup" only:
                user | project
  --skill-target
                User-scope skills target for "omxc setup" only:
                codex-home
`;
...
const REASONING_USAGE = "Usage: omxc reasoning <low|medium|high|xhigh>";
...
function runCodexBlocking(
  ...
  if (result.error) {
    ...
    if (kind === "missing") {
      console.error(
        "[omxc] failed to launch codex: executable not found in PATH",
      );
    } else if (kind === "blocked") {
      console.error(
        `[omxc] failed to launch codex: executable is present but blocked in the current environment (${errno.code || "blocked"})`,
      );
    } else {
      console.error(`[omxc] failed to launch codex: ${errno.message}`);
    }
    throw result.error;
  }
  ...
  if (result.status !== 0) {
    ...
    if (result.signal) {
      console.error(`[omxc] codex exited due to signal ${result.signal}`);
    }
  }
}
...
export async function launchWithHud(args: string[]): Promise<void> {
  if (isNativeWindows()) {
    ...
    if (result.error) {
      ...
      if (kind === "missing") {
        console.warn(
          "[omxc] warning: tmux was not found on native Windows. Continuing without tmux/HUD.\n" +
          "[omxc] To enable tmux-backed features, install psmux:\n" +
          "[omxc]   winget install psmux\n" +
          "[omxc] See: https://github.com/marlocarlo/psmux",
        );
      } else {
        console.warn(
          `[omxc] warning: tmux probe failed on native Windows (${errno.code || errno.message}). Continuing without tmux/HUD.`,
        );
      }
    } else if (result.status !== 0 && !isTmuxAvailable()) {
      const stderr = (result.stderr || "").trim();
      console.warn(
        `[omxc] warning: tmux reported an error on native Windows${stderr ? ` (${stderr})` : ""}. Continuing without tmux/HUD.`,
      );
    }
  }
  ...
  // After an omxc version upgrade the OLD setup code (still in memory) may
  ...
  try {
    ...
    if (repaired) {
      console.log("[omxc] Repaired managed config.toml compatibility issue.");
    }
  } catch {
  ...
  try {
    ...
    console.error(
      `[omxc] preLaunch warning: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  for (const stateDir of scopedDirs) {
    ...
    for (const file of files) {
      ...
      if (result.kind !== "ok") {
        if (result.kind === "recoverable") {
          try {
            ...
            writeWarn(
              `[omxc] postLaunch: failed to recover mode state ${path}: ${err instanceof Error ? err.message : err}`,
            );
          }
        } else if (result.kind === "malformed") {
          writeWarn(
            `[omxc] postLaunch: skipped malformed mode state ${path}: ${result.message}`,
          );
        }
        continue;
      }
      ...
      try {
        ...
        writeWarn(
          `[omxc] postLaunch: failed to update mode state ${path}: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
  }
...
export async function reapPostLaunchOrphanedMcpProcesses(
  ...
  try {
    ...
    if (result.terminatedCount > 0) {
      writeInfo(
        `[omxc] postLaunch: reaped ${result.terminatedCount} orphaned OMX MCP process(es).`,
      );
    }
    if (result.failedPids.length > 0) {
      writeWarn(
        `[omxc] postLaunch: failed to reap ${result.failedPids.length} orphaned OMX MCP process(es); continuing cleanup.`,
      );
    }
  } catch (err) {
...
async function preLaunch(
  ...
  try {
    ...
    if (cleanup.terminatedCount > 0) {
      console.log(
        `[omxc] Reaped ${cleanup.terminatedCount} orphaned OMX MCP process(es) before launch.`,
      );
    }
    if (cleanup.failedPids.length > 0) {
      console.warn(
        `[omxc] Failed to reap ${cleanup.failedPids.length} orphaned OMX MCP process(es); continuing launch.`,
      );
    }
  } catch (err) {
    process.stderr.write(`[cli/index] operation failed: ${err}\n`);
  ...
  try {
    if (notifyTempContract?.active) {
      ...
      for (const info of startup.infoLines) {
        console.log(`[omxc] ${info}`);
      }
      for (const warning of startup.warningLines) {
        console.warn(`[omxc] ${warning}`);
      }
    } else {
...
async function postLaunch(
  ...
  try {
    ...
    console.error(
      `[omxc] postLaunch: model instructions cleanup failed: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    console.error(
      `[omxc] postLaunch: session archive failed: ${err instanceof Error ? err.message : err}`,
    );
  }
  ...
  try {
    ...
    console.error(
      `[omxc] postLaunch: mode cleanup failed: ${err instanceof Error ? err.message : err}`,
    );
  }
...
export async function reapStaleNotifyFallbackWatcher(
  ...
  try {
    ...
    if (!hasErrnoCodeImpl(error, "ESRCH")) {
      warn(
        "[omxc] warning: failed to stop stale notify fallback watcher",
        {
          path: pidPath,
...
async function startNotifyFallbackWatcher(
  ...
  await mkdir(join(cwd, ".omx", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
        "[omxc] warning: failed to create notify fallback watcher state directory",
        {
          cwd,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  try {
    ...
    console.warn("[omxc] warning: failed to launch notify fallback watcher", {
      cwd,
      error: error instanceof Error ? error.message : String(error),
    });
    return;
  ...
  await writeFile(
    ...
    console.warn(
      "[omxc] warning: failed to write notify fallback watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function startHookDerivedWatcher(cwd: string): Promise<void> {
  ...
  if (existsSync(pidPath)) {
    try {
      ...
      console.warn("[omxc] warning: failed to stop stale hook-derived watcher", {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  await mkdir(join(cwd, ".omx", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
        "[omxc] warning: failed to create hook-derived watcher state directory",
        {
          cwd,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  try {
    ...
    console.warn("[omxc] warning: failed to launch hook-derived watcher", {
      cwd,
      error: error instanceof Error ? error.message : String(error),
    });
    return;
  ...
  await writeFile(
    ...
    console.warn(
      "[omxc] warning: failed to write hook-derived watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function stopNotifyFallbackWatcher(cwd: string): Promise<void> {
  ...
  try {
    ...
    if (!hasErrnoCode(error, "ESRCH")) {
      console.warn(
        "[omxc] warning: failed to stop notify fallback watcher process",
        {
          path: pidPath,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  await unlink(pidPath).catch((error: unknown) => {
    console.warn(
      "[omxc] warning: failed to remove notify fallback watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
...
async function stopHookDerivedWatcher(cwd: string): Promise<void> {
  ...
  try {
    ...
    console.warn("[omxc] warning: failed to stop hook-derived watcher process", {
      path: pidPath,
      error: error instanceof Error ? error.message : String(error),
    });
  }
  ...
  await unlink(pidPath).catch((error: unknown) => {
    console.warn(
      "[omxc] warning: failed to remove hook-derived watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
