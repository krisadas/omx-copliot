 ...
 *
 * This stage delegates to the existing `omxc team` infrastructure, which
...
/**
 * Build the `omxc team` CLI instruction from a descriptor.
 */
export function buildTeamInstruction(descriptor: TeamExecDescriptor): string {
  const launchCommand = `omxc team ${descriptor.workerCount}:${descriptor.agentType} ${JSON.stringify(descriptor.task)}`;
