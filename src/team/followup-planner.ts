...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `omxc team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
  ...
  return {
    shellCommand: `omxc ralph ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `omxc team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.',
