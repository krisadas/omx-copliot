...
async function assertTeamStartupIsNonDestructive(
  ...
  const tmuxSession = existingConfig?.tmux_session ?? existingManifest?.tmux_session ?? `omxc-team-${teamName}`;
  ...
  throw new Error(
    ...
    + `Use "omxc team status ${teamName}", "omxc team resume ${teamName}", or "omxc team shutdown ${teamName}" instead of launching a duplicate team.`,
  );
}
...
function summarizeWorktreeDiffWithSparkShell(worktreePath: string): string | null {
  ...
  const result = runCommand('omxc', ['sparkshell', 'sh', '-lc', shellCommand], worktreePath);
...
function doesStartupEvidenceSettle(
  ...
  if (workerCli === 'copilot-cli' && evidence === 'leader_ack') return false;
  return true;
}
...
function spawnPromptWorker(
  ...
  workerCli: 'copilot-cli' | 'claude' | 'gemini',
...
function resolveEffectiveWorkerCliForStartupLog(
  ...
  if (rawCliMap !== '') {
    ...
    if (entries.length > 0) {
      ...
      const resolvedMap = entries.map((entry): 'copilot-cli' | 'claude' | 'gemini' | null => {
        if (entry === 'auto') return autoCli;
        if (entry === 'copilot-cli' || entry === 'claude' || entry === 'gemini') return entry;
        return null;
      });
      if (resolvedMap.every((entry) => entry === 'claude')) return 'claude';
      if (resolvedMap.every((entry) => entry === 'gemini')) return 'gemini';
      if (resolvedMap.some((entry) => entry === 'copilot-cli')) return 'copilot-cli';
    }
  }
...
export async function startTeam(
  ...
  let sessionName = `omxc-team-${sanitized}`;
  ...
  try {
    ...
    for (let i = 1; i <= workerCount; i++) {
      ...
      const workerRole = taskRoles.length > 0 && uniqueTaskRoles.size === 1
        ? taskRoles[0]
        : agentType;
      const rawRolePromptContent = await loadRolePrompt(workerRole, join(leaderCwd, '.copilot', 'prompts'))
...
export async function shutdownTeam(teamName: string, cwd: string, options: ShutdownOptions = {}): Promise<TeamShutdownSummary> {
  ...
  if (!config) {
    ...
    try {
      destroyTeamSession(`omxc-team-${sanitized}`);
    } catch (err) {
      process.stderr.write(`[team/runtime] operation failed: ${err}\n`);
    }
...
async function findActiveTeams(cwd: string, leaderSessionId: string): Promise<string[]> {
  ...
  for (const e of entries) {
    ...
    const workerLaunchMode = cfg?.worker_launch_mode
      ...
      ?? 'interactive';
    const tmuxSession = (manifest?.tmux_session || cfg?.tmux_session || `omxc-team-${teamName}`).split(':')[0];
    if (leaderSessionId) {
...
async function dispatchCriticalInboxInstruction(params: {
  ...
  const requiresObservedStartupEvidence = requireWorkerStartupEvidence === true
    && (workerCli === 'claude' || workerCli === 'copilot-cli');
