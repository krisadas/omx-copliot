...
export async function readTeamPaneStatus(
  ...
  const sparkshellCommands = Object.fromEntries(
    [
      leaderPaneId ? ['leader', `omxc sparkshell --tmux-pane ${leaderPaneId} --tail-lines ${tailLines}`] : null,
      hudPaneId ? ['hud', `omxc sparkshell --tmux-pane ${hudPaneId} --tail-lines ${tailLines}`] : null,
      ...Object.entries(workerPanes).map(([workerName, paneId]) => [
        workerName,
        `omxc sparkshell --tmux-pane ${paneId} --tail-lines ${tailLines}`,
      ] as const),
    ].filter((entry): entry is [string, string] => entry !== null),
  );
  ...
  return {
    ...
    sparkshell_hint: Object.keys(workerPanes).length > 0
      ? 'omxc sparkshell --tmux-pane <pane-id> --tail-lines 400'
      : null,
