...
export type TeamWorkerCli = 'copilot-cli' | 'claude' | 'gemini';
...
function normalizeTeamWorkerCliMode(raw: string | undefined, sourceEnv: string = OMX_TEAM_WORKER_CLI_ENV): TeamWorkerCliMode {
  ...
  if (normalized === 'copilot-cli' || normalized === 'claude' || normalized === 'gemini') return normalized;
...
export function translateWorkerLaunchArgsForCli(workerCli: TeamWorkerCli, args: string[], initialPrompt?: string): string[] {
  if (workerCli === 'copilot-cli') return [...args];
  if (workerCli === 'gemini') {
...
export function buildWorkerSubmitPlan(
  ...
  return {
    ...
    queueFirstRound: workerCli === 'copilot-cli' && queueRequested,
    rounds: 6,
    submitKeyPressesPerRound: workerCli === 'claude' ? 1 : 2,
    allowAdaptiveRetry: workerCli === 'copilot-cli' && allowAdaptiveRetry,
  };
}
...
// List all tmux sessions matching omxc-team-* pattern
 ...
 * workers and runtime paths may message `leader-fixed` via `omxc team api`
