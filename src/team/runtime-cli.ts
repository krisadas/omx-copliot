...
type TeamWorkerProvider = 'copilot-cli' | 'claude' | 'gemini';
...
export function buildTerminalCliResult(
  ...
  return {
    ...
    notice: `[runtime-cli] phase=${phase} reached terminal state; preserving team state for inspection. Run "omxc team shutdown ${teamName}" when explicit cleanup is desired.\n`,
  };
}
...
export function normalizeAgentTypes(raw: string[], workerCount: number): TeamWorkerProvider[] {
  ...
  const invalid = providers.filter((entry) => entry !== 'copilot-cli' && entry !== 'claude' && entry !== 'gemini');
  if (invalid.length > 0) {
