...
describe('idle-nudge', () => {
  ...
  it('throttles scans that happen too soon after the previous scan', async () => {
    await withFakeTmux(async ({ tmuxLogPath }) => {
      await withMockedNow(10_000, async (setNow) => {
        ...
        const first = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
        assert.deepEqual(first, ['%2']);
        const firstLog = await readFile(tmuxLogPath, 'utf-8');

        setNow(11_000); // < 5000ms scan interval
        const second = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
  ...
  it('never nudges the leader pane', async () => {
    await withFakeTmux(async ({ tmuxLogPath }) => {
      const tracker = new NudgeTracker({ delayMs: 0, maxCount: 3, message: 'nudge' });
      const nudged = await tracker.checkAndNudge(['%1'], '%1', 'omxc-team-a');
  ...
  it('resets idle timer when pane becomes active before delay elapses', async () => {
    await withFakeTmux(async ({ setCaptureSequence }) => {
      ...
      await withMockedNow(10_000, async (setNow) => {
        ...
        const r1 = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
        assert.deepEqual(r1, []);

        setNow(16_000);
        const r2 = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
        assert.deepEqual(r2, []);

        setNow(22_000);
        const r3 = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
        assert.deepEqual(r3, []);

        setNow(28_000);
        const r4 = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
        assert.deepEqual(r4, []);

        setNow(39_000);
        const r5 = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
  ...
  it('does not count nudges when sendToWorker fails', async () => {
    await withFakeTmux(async () => {
      ...
      await withMockedNow(10_000, async () => {
        const tracker = new NudgeTracker({ delayMs: 0, maxCount: 3, message: 'nudge' });
        const nudged = await tracker.checkAndNudge(['%2'], undefined, 'omxc-team-a');
        assert.deepEqual(nudged, []);
        assert.equal(tracker.totalNudges, 0);
        assert.deepEqual(tracker.getSummary(), {});
      });
