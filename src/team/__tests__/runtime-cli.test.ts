...
describe('runtime-cli helpers', () => {
  it('normalizes per-worker providers and validates supported values', async () => {
    ...
    assert.deepEqual(
      runtimeCli.normalizeAgentTypes(['copilot-cli', 'gemini'], 2),
      ['copilot-cli', 'gemini'],
    );
    assert.deepEqual(
    ...
    assert.throws(
      () => runtimeCli.normalizeAgentTypes(['copilot-cli', 'invalid'], 2),
  ...
  it('preserves team state when building terminal output for completed teams', async () => {
    ...
    try {
      ...
      assert.match(result.notice, /omxc team shutdown runtime-cli-preserve-complete/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('preserves team state when building terminal output for failed teams', async () => {
    ...
    try {
      ...
      assert.match(result.notice, /preserving team state/i);
      assert.match(result.notice, /omxc team shutdown runtime-cli-preserve-failed/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('reports cancelled terminal phases without deleting team state', async () => {
    ...
    try {
      ...
      assert.match(result.notice, /omxc team shutdown runtime-cli-preserve-cancelled/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
