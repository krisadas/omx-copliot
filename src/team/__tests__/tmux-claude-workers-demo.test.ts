...
describe('tmux claude workers demo', () => {
  describe('mixed worker CLI map (like demo-team-e2e.sh)', () => {
    it('builds default CLI map with codex first half, claude second half', () => {
      ...
      assert.deepEqual(entries, ['copilot-cli', 'copilot-cli', 'copilot-cli', 'claude', 'claude', 'claude']);
      ...
      assert.deepEqual(plan, ['copilot-cli', 'copilot-cli', 'copilot-cli', 'claude', 'claude', 'claude']);
    });
    ...
    it('handles odd worker count correctly (5 workers)', () => {
      ...
      const entries: string[] = [];
      for (let i = 1; i <= workerCount; i++) {
        entries.push(i <= pivot ? 'copilot-cli' : 'claude');
      }

      assert.deepEqual(entries, ['copilot-cli', 'copilot-cli', 'copilot-cli', 'claude', 'claude']);

      const plan = resolveTeamWorkerCliPlan(workerCount, [], {
        OMX_TEAM_WORKER_CLI_MAP: entries.join(','),
      });

      assert.deepEqual(plan, ['copilot-cli', 'copilot-cli', 'copilot-cli', 'claude', 'claude']);
    });
  ...
  describe('translateWorkerLaunchArgsForCli', () => {
    it('returns codex args unchanged', () => {
      ...
      const result = translateWorkerLaunchArgsForCli('copilot-cli', args);
  ...
  describe('full demo scenario with 6 mixed workers', () => {
    ...
    it('handles auto-resolved codex from launch args', () => {
      // When launch args contain 'copilot-cli' in model name and no CLI_MAP is set
      const plan = resolveTeamWorkerCliPlan(2, ['--model', 'gpt-5.3-codex'], {});

      // Both should resolve to codex because model name contains 'copilot-cli' but not 'claude'
      assert.deepEqual(plan, ['copilot-cli', 'copilot-cli']);
    });
  });
  ...
  describe('edge cases for mixed worker teams', () => {
    ...
    it('handles single worker with codex', () => {
      const plan = resolveTeamWorkerCliPlan(1, [], {
        OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli',
      });

      assert.deepEqual(plan, ['copilot-cli']);
    });
