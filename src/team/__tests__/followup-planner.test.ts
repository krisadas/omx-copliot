...
describe('followup-planner', () => {
  ...
  it('builds concrete team staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omxc team 3:executor "Fix flaky integration tests and update README"');
  ...
  it('builds concrete ralph staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omxc ralph "Investigate auth regression and verify the fix"');
