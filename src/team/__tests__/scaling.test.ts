...
describe('scaleUp', () => {
  ...
  it('persists scaled-up task roles in canonical task state and inbox ids', async () => {
    ...
    try {
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omxc-team-scale-up-role';
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omxc-team-rollback-worktree';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omxc-team-canonical-root';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omxc-team-frontier-role';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omxc-team-mini-tuned-root';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omxc-team-scale-up-layout';
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
      await saveTeamConfig(config, cwd);
  ...
  it('provisions detached worktrees for scaled-up workers from persisted team worktree mode', async () => {
    ...
    try {
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = `omxc-team-${teamName}`;
      config.leader_pane_id = '%11';
      config.workers[0]!.pane_id = '%21';
