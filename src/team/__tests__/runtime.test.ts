...
describe('runtime', () => {
  ...
  it('resolveWorkerLaunchArgsFromEnv keeps copilot-cli thinking_level logging for mixed CLI maps', () => {
    const logs: string[] = [];
    const originalLog = console.log;
    console.log = (...args: unknown[]) => { logs.push(args.join(' ')); };
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        {
          OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli,claude',
...
`,
  ...
  it('skips interactive worker process-tree prekill on native Windows split-pane sessions', async () => {
    await withNativeWindowsPlatform(async () => {
      ...
      assert.equal(shouldPrekillInteractiveShutdownProcessTrees('omxc-team-alpha'), true);
    });
...
`,
          binaries: [{ name: 'copilot-cli', content: '#!/bin/sh\nexit 0\n' }],
        },
        async () => {
          delete process.env.TMUX;
...
`,
  ...
  it('shutdownTeam handles persisted resize hook metadata during cleanup', async () => {
    ...
    try {
      ...
      config.resize_hook_target = 'omxc-team-team-resize-meta:0';
      await writeFile(configPath, JSON.stringify(config, null, 2));
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
      manifest.resize_hook_name = 'omx_resize_team_resize_meta_test';
      manifest.resize_hook_target = 'omxc-team-team-resize-meta:0';
      await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
...
case "$1" in
  -V)
    ...
    ;;
  list-sessions)
    echo "omxc-team-team-shutdown-gate-failed"
    exit 0
    ;;
  set-hook)
    if [ "\${2:-}" = "-u" ]; then
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          const config = JSON.parse(await readFile(configPath, 'utf-8')) as Record<string, unknown>;
          config.tmux_session = 'omxc-team-team-shutdown-gate-failed';
          config.resize_hook_name = 'omx_resize_team_shutdown_gate_failed_test';
          config.resize_hook_target = 'omxc-team-team-shutdown-gate-failed:0';
          await writeFile(configPath, JSON.stringify(config, null, 2));
          const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
          manifest.tmux_session = 'omxc-team-team-shutdown-gate-failed';
          manifest.resize_hook_name = 'omx_resize_team_shutdown_gate_failed_test';
          manifest.resize_hook_target = 'omxc-team-team-shutdown-gate-failed:0';
          await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
          ...
          const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
          assert.match(tmuxLog, /set-hook -u -t omxc-team-team-shutdown-gate-failed:0 client-resized\[\d+\]/);
          assert.match(tmuxLog, /kill-session -t omxc-team-team-shutdown-gate-failed/);
        },
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          assert.ok(config);
          if (!config) return;
          config.tmux_session = 'omxc-team-team-shutdown-dead-pane';
          ...
          assert.match(tmuxLog, /kill-session -t omxc-team-team-shutdown-dead-pane/);
        },
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          assert.ok(config);
          if (!config) return;
          config.tmux_session = 'omxc-team-team-shutdown-exclusions';
          config.leader_pane_id = '%11';
          config.hud_pane_id = '%12';
