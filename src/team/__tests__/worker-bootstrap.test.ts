...
describe("worker bootstrap", () => {
  it("worker skill lifecycle instructions are claim-safe (issue #448)", async () => {
    ...
    assert.match(workerSkill, /omxc team api claim-task/);
    assert.match(workerSkill, /omxc team api transition-task-status/);
    assert.match(workerSkill, /omxc team api release-task-claim/);
    assert.match(
      workerSkill,
  ...
  it("generateWorkerOverlay includes the team name", () => {
    ...
    assert.match(overlay, /omxc team api claim-task/);
    assert.match(overlay, /omxc team api transition-task-status/);
    assert.match(overlay, /omxc team api release-task-claim/);
    assert.doesNotMatch(
      overlay,
  ...
  it("generateInitialInbox includes worker name, team name, and all tasks", () => {
    ...
    assert.match(inbox, /omxc team api claim-task/);
    assert.match(inbox, /omxc team api transition-task-status/);
    assert.match(inbox, /omxc team api release-task-claim/);
    assert.match(
      inbox,
      /\$\{CODEX_HOME:-~\/\.codex\}\/skills\/worker\/SKILL\.md/,
    );
