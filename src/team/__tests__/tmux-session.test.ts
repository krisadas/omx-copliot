...
describe('chooseTeamLeaderPaneId', () => {
  it('keeps preferred pane when it is not HUD', () => {
    const panes = [
      { paneId: '%1', currentCommand: 'node', startCommand: "'copilot-cli'" },
      { paneId: '%2', currentCommand: 'node', startCommand: "node omxc hud --watch" },
    ];
  ...
  it('falls back to preferred pane when all panes are HUD panes', () => {
    const panes = [
      { paneId: '%2', currentCommand: 'node', startCommand: "node omxc hud --watch" },
      { paneId: '%3', currentCommand: 'node', startCommand: "node omxc hud --watch" },
    ];
...
describe('sendToWorker validation', () => {
  it('rejects text over 200 chars', async () => {
    await assert.rejects(
      sendToWorker('omxc-team-x', 1, 'a'.repeat(200)),
      /< 200/i
    );
  });
  ...
  it('rejects injection marker', async () => {
    await assert.rejects(
      sendToWorker('omxc-team-x', 1, `hello [OMX_TMUX_INJECT]`),
      /marker/i
    );
  });
...
`,
      async ({ logPath }) => {
        await sendToWorker('omxc-team-x', 1, 'check inbox');
        const log = await readFile(logPath, 'utf-8');
        const acceptIndex = log.indexOf('send-keys -t omxc-team-x:1 -l -- 2');
        const submitIndex = log.indexOf('send-keys -t omxc-team-x:1 -l -- check inbox');
...
describe('buildWorkerStartupCommand', () => {
  ...
  it('respects explicit OMX_TEAM_WORKER_CLI override', () => {
    ...
    try {
      ...
      assert.match(codexCmd, /exec .*copilot-cli/);
  ...
  it('uses zsh with ~/.zshrc and exec codex', () => {
    ...
    try {
      ...
      assert.match(cmd, /exec .*copilot-cli/);
    } finally {
      if (typeof prevShell === 'string') process.env.SHELL = prevShell;
      else delete process.env.SHELL;
      if (typeof prevBypass === 'string') process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
  ...
  it('resolves POSIX leader paths before building fish worker startup commands', async () => {
    ...
    try {
      ...
      const codexPath = join(fakeBin, 'copilot-cli');
  ...
  it('forces codex bypass under explicit launch-arg profiles', () => {
    ...
    try {
      ...
      for (const launchArgs of profiles) {
        const cmd = buildWorkerStartupCommand('alpha', 1, launchArgs, process.cwd(), {}, 'copilot-cli');
        assert.match(cmd, /exec .*copilot-cli/);
  ...
  it('supports worker-specific reasoning overrides for codex and strips them for claude workers', () => {
    ...
    process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = '0';
    try {
      const codexCmd = buildWorkerStartupCommand('alpha', 1, ['-c', 'model_reasoning_effort="low"'], process.cwd(), {}, 'copilot-cli');
  ...
  it('uses a native PowerShell startup command on native Windows instead of /bin/sh -lc', async () => {
    ...
    Object.defineProperty(process, 'platform', { value: 'win32', configurable: true });
    try {
      const codexPs1Path = join(fakeBin, 'copilot-cli.ps1');
...
describe('team worker CLI helpers', () => {
  it('resolveTeamWorkerCli auto-detects claude models', () => {
    ...
    assert.equal(resolveTeamWorkerCli(['--model', 'gpt-5'], {}), 'copilot-cli');
    assert.equal(resolveTeamWorkerCli([], {}), 'copilot-cli');
  });
  ...
  it('resolveTeamWorkerCliPlan accepts gemini in CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(3, [], { OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli,gemini,claude' });
    assert.deepEqual(plan, ['copilot-cli', 'gemini', 'claude']);
  });

  it('translateWorkerLaunchArgsForCli preserves args for codex', () => {
    const args = ['--model', 'gpt-5', '-c', 'model_reasoning_effort="xhigh"'];
    assert.deepEqual(translateWorkerLaunchArgsForCli('copilot-cli', args), args);
  });
  ...
  it('resolveTeamWorkerCliPlan supports mixed per-worker CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      4,
      [],
      { OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli,copilot-cli,gemini,claude' },
    );
    assert.deepEqual(plan, ['copilot-cli', 'copilot-cli', 'gemini', 'claude']);
  });
  ...
  it('resolveTeamWorkerCliPlan supports auto entries in CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      2,
      ['--model', 'claude-3-7-sonnet'],
      { OMX_TEAM_WORKER_CLI_MAP: 'auto,copilot-cli' },
    );
    assert.deepEqual(plan, ['claude', 'copilot-cli']);
  });
  ...
  it('resolveTeamWorkerCliPlan auto entries ignore OMX_TEAM_WORKER_CLI override', () => {
    const plan = resolveTeamWorkerCliPlan(
      ...
      ['--model', 'claude-3-7-sonnet'],
      {
        OMX_TEAM_WORKER_CLI: 'copilot-cli',
  ...
  it('resolveTeamWorkerCliPlan rejects map lengths that do not match workerCount', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(4, [], { OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli,claude' }),
      /expected 1 or 4/i,
    );
  });
  ...
  it('resolveTeamWorkerCliPlan rejects empty entries in CLI map', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(2, [], { OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli,' }),
  ...
  it('resolveWorkerCliForSend prioritizes explicit worker CLI over map/global', () => {
    assert.equal(
      resolveWorkerCliForSend(2, 'claude', [], { OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli,copilot-cli' }),
      'claude',
    );
  });
  ...
  it('resolveWorkerCliForSend resolves per-worker map entry by index', () => {
    assert.equal(
      resolveWorkerCliForSend(2, undefined, [], { OMX_TEAM_WORKER_CLI_MAP: 'copilot-cli,claude' }),
      'claude',
    );
  });
  ...
  it('buildWorkerSubmitPlan preserves queue-first behavior for busy codex workers', () => {
    const plan = buildWorkerSubmitPlan('auto', 'copilot-cli', true, true);
...
describe('team worker launch mode helpers', () => {
  ...
  it('buildWorkerProcessLaunchSpec returns command/args/env for prompt process spawn', () => {
    ...
    try {
      ...
      assert.equal(spec.workerCli, 'copilot-cli');
  ...
  it('buildWorkerProcessLaunchSpec injects the active provider env_key from CODEX_HOME config.toml', async () => {
    ...
    const codexHome = await mkdtemp(join(tmpdir(), 'omxc-team-provider-env-'));
    process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = '0';
  ...
  it('buildWorkerProcessLaunchSpec reads provider env from worker CODEX_HOME override', async () => {
    ...
    const leaderCodexHome = await mkdtemp(join(tmpdir(), 'omxc-team-provider-env-leader-'));
    const workerCodexHome = await mkdtemp(join(tmpdir(), 'omxc-team-provider-env-worker-'));
    process.env.OMX_BYPASS_DEFAULT_SYSTEM_PROMPT = '0';
  ...
  it('buildWorkerProcessLaunchSpec resolves relative worker CODEX_HOME against the worker cwd', async () => {
    ...
    const leaderCwd = await mkdtemp(join(tmpdir(), 'omxc-team-provider-relative-leader-'));
    const workerCwd = await mkdtemp(join(tmpdir(), 'omxc-team-provider-relative-worker-'));
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omxc-team-x', 1, 1_000), true);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /capture-pane -t omxc-team-x:1 -p/);
        assert.doesNotMatch(log, /capture-pane -t omxc-team-x:1 -p -S/);
      },
    );
  });
...
`,
      async ({ logPath }) => {
        ...
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /capture-pane -t omxc-team-x:1 -p/);
        assert.match(log, /capture-pane -t omxc-team-x:1 -p -S -80/);
      },
    );
  });
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omxc-team-x', 1, 250), false);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /capture-pane -t omxc-team-x:1 -p/);
        assert.doesNotMatch(log, /capture-pane -t omxc-team-x:1 -p -S -80/);
      },
    );
  });
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omxc-team-x', 1, 1_000), true);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /send-keys -t omxc-team-x:1 -l -- 2/);
        assert.match(log, /send-keys -t omxc-team-x:1 C-m/);
      },
    );
  });
...
`,
  ...
  it('waitForWorkerReady returns false on timeout', () => {
    withEmptyPath(() => {
      assert.equal(waitForWorkerReady('omxc-team-x', 1, 1), false);
    });
  });
});
...
describe('native Windows HUD reconciliation', () => {
  it('allows team startup on native Windows when current tmux client is reachable without TMUX env vars', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omxc-team-win32-no-env-'));
...
`,
  ...
  it('avoids nested tmux run-shell hooks during team HUD startup on native Windows', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omxc-team-win32-hud-'));
    const prevTmux = process.env.TMUX;
    const prevTmuxPane = process.env.TMUX_PANE;
    const prevWorkerCli = process.env.OMX_TEAM_WORKER_CLI;
    const prevMsystem = process.env.MSYSTEM;
...
`,
        async ({ logPath }) => {
          assert.equal(dismissTrustPromptIfPresent('omxc-team-x', 1), true);
          const log = await readFile(logPath, 'utf-8');
          assert.match(log, /capture-pane -t omxc-team-x:1 -p/);
          assert.doesNotMatch(log, /capture-pane -t omxc-team-x:1 -p -S/);
        },
...
describe('dismissTrustPromptIfPresent', () => {
  it('returns false when tmux is unavailable', () => {
    withEmptyPath(() => {
      assert.equal(dismissTrustPromptIfPresent('omxc-team-x', 1), false);
    });
  });
...
describe('isWorkerAlive', () => {
  it('does not require pane_current_command to match "copilot-cli"', () => {
    ...
    withEmptyPath(() => {
      assert.equal(isWorkerAlive('omxc-team-x', 1), false);
    });
  });
});
...
describe('enableMouseScrolling', () => {
  it('returns false when tmux is unavailable', () => {
    ...
    withEmptyPath(() => {
      assert.equal(enableMouseScrolling('omxc-team-x'), false);
    });
  });
...
describe('enableMouseScrolling scroll and copy setup (issue #206)', () => {
  ...
  it('does not throw when WSL2 env is set and tmux is unavailable (regression + #206)', () => {
    ...
    try {
      withEmptyPath(() => {
        assert.doesNotThrow(() => enableMouseScrolling('omxc-team-x'));
      });
    } finally {
      if (typeof prev === 'string') process.env.WSL_DISTRO_NAME = prev;
      else delete process.env.WSL_DISTRO_NAME;
...
`,
      async ({ logPath }) => {
        assert.equal(enableMouseScrolling('omxc-team-x'), true);
        const tmuxLog = await readFile(logPath, 'utf-8');
        assert.match(tmuxLog, /set-option -t omxc-team-x mouse on/);
        assert.match(tmuxLog, /set-option -t omxc-team-x set-clipboard on/);
...
describe('killWorker leader pane guard', () => {
  it('returns immediately when workerPaneId matches leaderPaneId', () => {
    ...
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omxc-team-x:0', 1, '%5', '%5'));
    });
  });
  ...
  it('proceeds (gracefully) when pane ids differ', () => {
    ...
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omxc-team-x:0', 1, '%5', '%6'));
    });
  });

  it('proceeds when leaderPaneId is not provided', () => {
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omxc-team-x:0', 1, '%5'));
    });
  });
});
