...
export function buildExploreRoutingGuidance(env: NodeJS.ProcessEnv = process.env): string {
  ...
  return [
    ...
    '- Advisory steering only: agents SHOULD treat `omxc explore` as the default first stop for direct inspection and SHOULD reserve `omxc sparkshell` for qualifying read-only shell-native tasks.',
    '- For simple file/symbol lookups, use `omxc explore` FIRST before attempting full code analysis.',
    '- When the user asks for a simple read-only exploration task (file/symbol/pattern/relationship lookup), strongly prefer `omxc explore` as the default surface.',
    '- Explore examples: `omxc explore --prompt "which files define TeamPolicy"`, `omxc explore --prompt "find usages of buildExploreRoutingGuidance"`.',
    '- SparkShell examples: use `omxc sparkshell -- rg -n "TeamPolicy" src`, `omxc sparkshell -- npm test`, or `omxc sparkshell --tmux-pane %12` for noisy verification, bounded shell output, or tmux-pane summaries.',
    '- Keep `omxc explore` prompts narrow and concrete; prefer a single lookup goal or a small related cluster, using `--prompt` for quick asks and `--prompt-file` for longer reusable briefs.',
    '- Treat `omxc explore` as a shell-only allowlisted read-only path; keep edits, tests, diagnostics, MCP/web needs, and complex shell composition on the richer normal path.',
    '- Keep implementation, refactor, test, or ambiguous broad requests on the normal Codex path.',
    '- If `omxc explore` is unavailable, stalls, or fails, retry with a narrower prompt or gracefully fall back to the normal path.',
  ].join("\n");
}
