...
`);
  ...
  describe('omx', () => {
    it('exposes only the explicit read-only omxc readers', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omx-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
    ...
    it('reads hud, notifyFallback, and updateCheck state from root-scoped omxc files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omx-sdk-'));
      try {
    ...
    it('returns null for missing omxc reader files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omx-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omx.session.read(), null);
