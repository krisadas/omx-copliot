...
describe('notify-hook leader-side authority handoff', () => {
  ...
  it('does not nudge stale leader when recent team status activity proves the leader is active', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-beta-active-status',
        leader_pane_id: '%92',
        workers: [
...
describe('notify-hook team leader nudge', () => {
  ...
  it('suggests shutdown when all workers are idle and the current task set is complete', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: decide whether to reconcile\/merge results or gracefully shut down: omxc team shutdown idle-shutdown\./);
...
`;
  ...
  it('surfaces ack-like mailbox replies without work-start evidence as missing-start nudges', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: check worker-1 msg\/output, confirm task in omxc team status ack-missing-start/);
  ...
  it('does not classify ack-like replies as missing-start after a worker has claimed work', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: read messages; keep orchestrating; if done, gracefully shut down: omxc team shutdown ack-with-start\./);
...
`;
  ...
  it('nudges when worker panes are alive and leader is stale (no recent HUD turn)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: check messages; keep orchestrating; if done, gracefully shut down: omxc team shutdown beta\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.match(tmuxLog, /\[OMX_TMUX_INJECT\]/, 'should include injection marker');
    });
  });
  ...
  it('nudges when team progress is stalled even if timing signals are fresh or missing (fallback threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-stalled-progress',
        leader_pane_id: '%90',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
      ...
      assert.match(tmuxLog, /Next: omxc team status stalled-progress; read worker messages; unblock\/reassign or shutdown\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.doesNotMatch(tmuxLog, /\[OMX_INTENT:/);
  ...
  it('nudges when team progress is stalled before the leader becomes stale (fallback threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-stalled-before-stale',
        leader_pane_id: '%89',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
      ...
      assert.match(tmuxLog, /Next: omxc team status stalled-before-stale; read worker messages; unblock\/reassign or shutdown\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.doesNotMatch(tmuxLog, /leader stale/);
  ...
  it('nudges stalled team after configurable worker turn stall window elapses (primary threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-worker-turn-stall-threshold',
        leader_pane_id: '%86',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
        ],
  ...
  it('does not nudge stalled team when an in-progress worker is still advancing heartbeat turns', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-active-turns-no-stall',
        leader_pane_id: '%87',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
  ...
  it('bounds repeated stalled-team nudges before leader stale by cooldown', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-stalled-before-stale-bounded',
        leader_pane_id: '%88',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
      ...
      assert.match(tmuxLog, /Next: omxc team status stalled-before-stale-bounded; read worker messages; unblock\/reassign or shutdown\./);
    });
  });
  ...
  it('does not treat leader and HUD panes as active worker panes when worker pane ids are known', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-stale-no-workers',
        leader_pane_id: '%92',
        hud_pane_id: '%93',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
  ...
  it('uses a 30s cadence for stale leader follow-up nudges', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omxc-team-stale-cadence',
        leader_pane_id: '%96',
      });
  ...
  it('includes stale_leader_with_messages reason when both conditions met', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: read messages; keep orchestrating; if done, gracefully shut down: omxc team shutdown delta\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.match(tmuxLog, /\[OMX_TMUX_INJECT\]/, 'should include injection marker');
