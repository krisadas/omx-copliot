...
describe('explore-routing', () => {
  ...
  it('builds advisory guidance whenever routing is not explicitly disabled', () => {
    ...
    assert.match(guidance, /agents SHOULD treat `omxc explore` as the default first stop/i);
    assert.match(guidance, /use `omxc explore` FIRST before attempting full code analysis/i);
