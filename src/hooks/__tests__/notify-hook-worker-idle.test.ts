...
`;
  ...
  it('existing all-workers-idle hook still fires alongside per-worker', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: run omxc team status both-hooks, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle nudge should include a next action');
    });
  });
});
