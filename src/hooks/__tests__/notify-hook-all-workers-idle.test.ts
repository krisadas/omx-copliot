...
`;
  ...
  it('handles single worker team correctly with singular message', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: run omxc team status solo-team, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle notification should include a next action');
